"""Baseline (pre-optimization) echo video dataloader, preserved for benchmarking.

This is a verbatim copy of the original ``LmdbEchoStudyVideoDataDescription`` as it
existed before the data-loading optimizations (metadata frame count, direct
``to_ndarray`` decode, multithreaded decode, cached LMDB envs). It is kept only so
the optimized version in ``echo.py`` can be timed head-to-head against it. Do not
...
class LmdbEchoStudyVideoDataDescriptionBaseline(DataDescription):
