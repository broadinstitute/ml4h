...
def main(
    ...
    OUTPUT_DD = EcholabDataDescription(
        wide_df=wide_df_selected[['sample_id'] + output_labels],  # .drop_duplicates(subset=['sample_id']),
    ...
    num_classes = [output_reg_len] + [cls_category_len_dict[c] if cls_category_len_dict[c]>2 else 1 for c in
                                      cls_category_map_dicts['cls_output_order']] if output_reg_len > 0 else [
        cls_category_len_dict[c] if cls_category_len_dict[c]>2 else 1 for c in cls_category_map_dicts['cls_output_order']]
