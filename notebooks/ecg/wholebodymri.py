...
for i, (sample_id, df_sample_id) in enumerate(df_manifest.groupby('sample_id')):
    ...
    try:
        ...
        hd5_ff.create_group('ukb_wholebody_mri')
        hd5_ff.close()
    for j, (instance, df_hd5) in enumerate(df_sample_id.groupby('instance')):
        try:
            raw_t1 = f'bodymri/all/raw/{sample_id}_20201_{instance}_0.zip'
            blob = bulk_bucket.blob(raw_t1)
            blob.download_to_filename('raw_wholebody.zip')
            with h5py.File(f'{sample_id}.hd5', 'a') as hd5_ff:
                with zipfile.ZipFile('raw_wholebody.zip', 'r') as zip_file:
