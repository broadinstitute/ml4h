...
df_covariates = df.dropna()
df_covariates['instance'] = df_covariates['instance'].apply(int)
df_covariates['sample_id'] =df_covariates['sample_id'].apply(int)

df_covariates = df_covariates.merge(covariates, on=['sample_id', 'instance'])

# %%
df_covariates[df_covariates['atrial_fibrillation_prevalent']>0.5][['sample_id', 'Interventricular Septum_model']].sort_values(by='Interventricular Septum_model', ascending=False).iloc[100:150]

# %%
df_covariates['dcm_prevalent'] = pd.to_datetime(df_covariates['mri_date'])
