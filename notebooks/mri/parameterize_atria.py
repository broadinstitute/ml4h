...
for t in range(0, MRI_FRAMES, 10):
    ...
    zs.append(np.cross(directions[1], directions[0]))
    xs.append(directions[0])
    ys.append(np.cross(zs[-1], xs[-1]))
    origins.append(np.mean(center, axis=0))
...
for t in range(0, MRI_FRAMES, 10):
    ...
    keep = np.dot((append_vertices[-1] - origins[t//10]), zs[t//10]) > 0.0
...
poissons = []
for t in range(0, MRI_FRAMES, 10):
    faces, vertices = poisson_reconstruction(append_kept_vertices[t//10], append_kept_normals[t//10])
    ...
    plane = vtk.vtkPlane()
    plane.SetOrigin(origins[t//10])
    plane.SetNormal(zs[t//10])
