...
# %%
df_la = pd.read_csv('/home/pdiachil/projects/la_3lax/all_LA_processed_fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122.csv')
...
labels = ['fastai-2ch-v20200809-3ch-v20200603-4ch-v20201122']
surfaces = ['poisson']

dfs_la = [df_la,]
for i, df_la in enumerate(dfs_la):
    dfs_la[i] = df_la[df_la['sample_id']!=-1]
    if 'instance' not in dfs_la[i]:
        dfs_la[i]['instance'] = 2

for label, surface, df_la in zip(labels, surfaces, dfs_la):
    keys = [f'LA_{surface}_{d}' for d in range(50)]
    keys_labels = [f'LA_{surface}_{d}_{label}' for d in range(50)]
    for key, key_label  in zip(keys, keys_labels):
        df_la[key_label] = df_la[key]
    df_la.drop(keys, axis=1, inplace=True)

axes = ['x_axis', 'y_axis', 'z_axis']

for label, surface, df_la in zip(labels*3, axes, dfs_la*3):
    keys = [f'LA_{surface}_{d}' for d in range(50)]
    keys_labels = [f'LA_{surface}_{d}_{label}' for d in range(50)]
    for key, key_label  in zip(keys, keys_labels):
        df_la[key_label] = df_la[key]
...
for label, surface in zip(labels, surfaces):
    keys.append([f'LA_{surface}_{d}_{label}' for d in range(50)])

key_triplets = [[keys[0][i], 
                #  keys[0][(i + 1) % len(keys[0])], 
                #  keys[0][(i + 2) % len(keys[0])],
                 ]
...
for label, surface, key in zip(labels, surfaces, keys):
    ...
    for key_triplet in key_triplets:
        df_all[key_triplet[0]] = np.nanmedian(df_all[key_triplet].values, axis=1)
    df_all[f'LAEDV_{surface}_{label}'] = df_all[key].max(axis=1)
    df_all[f'LAESV_{surface}_{label}'] = df_all[key].min(axis=1)
    df_all[f'LAEF_{surface}_{label}'] = (df_all[f'LAEDV_{surface}_{label}'] - df_all[f'LAESV_{surface}_{label}']) / df_all[f'LAEDV_{surface}_{label}'] * 100.

# %%
import numpy as np
for label, surface in zip(labels*3, axes):
    ed = df_all[keys[0]].values.argmax(axis=1)
    es = df_all[keys[0]].values.argmin(axis=1)
    keys_axis = [f'LA_{surface}_{d}_{label}' for d in range(50)]
    key_triplets = [[keys_axis[i], 
                    #  keys_axis[(i + 1) % len(keys_axis)], 
                    #  keys_axis[(i + 2) % len(keys_axis)],
                   ]
                                for i in range(len(keys_axis))]
    for key_triplet in key_triplets:
        df_all[key_triplet[0]].replace(-1.0, np.nan, inplace=True)
    for key_triplet in key_triplets:
        df_all[key_triplet[0]] = np.nanmedian(df_all[key_triplet].values, axis=1)
    df_all[f'LA_ED_{surface}_{label}'] = df_all[keys_axis].values[np.arange(len(df_all)), ed]
    df_all[f'LA_ES_{surface}_{label}'] = df_all[keys_axis].values[np.arange(len(df_all)), es]
    df_all[f'LA_EF_{surface}_{label}'] = (df_all[f'LA_ED_{surface}_{label}'] - df_all[f'LA_ES_{surface}_{label}']) / df_all[f'LA_ED_{surface}_{label}'] * 100.
...
# labels += ['mean']
# surfaces += ['poisson']
subset = [f'LAESV_poisson_{label}' for label in labels]
subset += [f'LAEDV_poisson_{label}' for label in labels]
subset += [f'LAEF_poisson_{label}' for label in labels]
for surface in axes:
    subset += [f'LA_ED_{surface}_{label}' for label in labels]
    subset += [f'LA_ES_{surface}_{label}' for label in labels]

df_inuse = df_all.dropna(subset=subset)
# %%
for feat in subset[:2]:
    df_inuse = df_inuse[df_inuse[feat] < 300]
    df_inuse = df_inuse[df_inuse[feat] > 5]
# %%
for feat in subset[3:]:
    df_inuse = df_inuse[df_inuse[feat] < 300]
    df_inuse = df_inuse[df_inuse[feat] > 5]

# %%

df_inuse.to_csv('/home/pdiachil/projects/la_3lax/all_LA_cleaned_fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122.csv', index=False)

f, ax = plt.subplots(3, 1)
sns.distplot(df_inuse[f'LAESV_poisson_{labels[0]}'], kde=False, ax=ax[0])
sns.distplot(df_inuse[f'LAEDV_poisson_{labels[0]}'], kde=False, ax=ax[1])
sns.distplot(df_inuse[f'LAEF_poisson_{labels[0]}'], kde=False, ax=ax[2])
# %%
f, ax = plt.subplots(3, 1)
sns.distplot(df_inuse[f'LA_ED_z_axis_{labels[0]}'], kde=False, ax=ax[0])
sns.distplot(df_inuse[f'LA_ES_z_axis_{labels[0]}'], kde=False, ax=ax[1])
sns.distplot(df_inuse[f'LA_EF_z_axis_{labels[0]}'], kde=False, ax=ax[2])

f, ax = plt.subplots(3, 1)
sns.distplot(df_inuse[f'LA_ED_y_axis_{labels[0]}'], kde=False, ax=ax[0])
sns.distplot(df_inuse[f'LA_ES_y_axis_{labels[0]}'], kde=False, ax=ax[1])
sns.distplot(df_inuse[f'LA_EF_y_axis_{labels[0]}'], kde=False, ax=ax[2])

f, ax = plt.subplots(3, 1)
sns.distplot(df_inuse[f'LA_ED_x_axis_{labels[0]}'], kde=False, ax=ax[0])
sns.distplot(df_inuse[f'LA_ES_x_axis_{labels[0]}'], kde=False, ax=ax[1])
sns.distplot(df_inuse[f'LA_EF_x_axis_{labels[0]}'], kde=False, ax=ax[2])
# %%
...
df_all_petersen = df_petersen.merge(df_all[df_all[f'instance']==2], on='sample_id')
...
subset += [f'LAEDV_poisson_{label}' for label in labels]
subset += [f'LAEF_poisson_{label}' for label in labels]
subset += ['LA_Biplan_vol_min', 'LA_Biplan_vol_max', 'LAEF']
df_all_petersen['LAEDV'] = df_all_petersen['LA_Biplan_vol_max']
df_all_petersen['LAESV'] = df_all_petersen['LA_Biplan_vol_min']
df_all_petersen['LAEF'] = (df_all_petersen['LA_Biplan_vol_max'] - df_all_petersen['LA_Biplan_vol_min']) / df_all_petersen['LA_Biplan_vol_max'] * 100.

df_inuse = df_all_petersen.dropna(subset=subset)
for feat in subset:
    df_inuse = df_inuse[df_inuse[feat] < 300]
    df_inuse = df_inuse[df_inuse[feat] > 5]
df_inuse.to_csv('/home/pdiachil/projects/la_3lax/all_LA_cleaned_petersen_fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122.csv', index=False)
f, ax = plt.subplots(3, (len(subset)-3)//3)
f.set_size_inches((len(subset)-3)//3*2.5, 6)



for i, (meas, extent) in enumerate(zip(['LAESV', 'LAEDV', 'LAEF'], [200, 400, 100])):
    for j, (surface, label) in enumerate(zip(surfaces, labels)):
        if len(labels) == 1:
            slices = i
        else:
            slices = (i, j)
        ax[slices].hexbin(df_inuse[f'{meas}_{surface}_{label}'], df_inuse[meas],
                        extent=(0, extent, 0, extent), mincnt=1, cmap='gray')
        ax[slices].set_aspect('equal')
        ax[slices].plot([0, 400], [0, 400], color='k')
        if i == 2:
            ax[slices].set_xlabel('')
        else:
            ax[slices].set_xlabel('')
        if j == 0:
            ax[slices].set_ylabel(meas)
        else:
            ax[slices].set_ylabel('')

        ax[slices].set_xlim([0, extent])
        ax[slices].set_ylim([0, extent])
        ...
        ax[slices].set_title(f'n={len(df_inuse)}, r$^2$={pearson*pearson:.3f}')

plt.tight_layout()
f.savefig('LA_petersen_all_20210204.png', dpi=500)

df_inuse.to_csv('/home/pdiachil/df_la_inuse_petersen.csv', index=False)
# %%
import scipy.stats
import matplotlib.pyplot as plt
