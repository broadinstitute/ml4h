...
# %%
df_la_old = pd.read_csv('/home/pdiachil/projects/surface_reconstruction/all_LAX_processed_v20201006.csv')
df_la_new = pd.read_csv('/home/pdiachil/projects/surface_reconstruction/fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122/all_LA_processed_fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122.csv')
...
labels = ['v20200809_v20200603_v20201122']
...
df_all = dfs_la[0]
label_left = labels[0]
for df_la, label_right in zip(dfs_la[1:], labels[1:]):
    df_all = df_all.merge(df_la, on='sample_id', suffixes=(f'_{label_left}', f'_{label_right}'))
...
for feat in subset[:-1]:
    df_inuse = df_inuse[df_inuse[feat] < 250]
    df_inuse = df_inuse[df_inuse[feat] > 5]
df_inuse.to_csv('/home/pdiachil/projects/surface_reconstruction/fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122/all_cleaned_LA_processed_fastai_sax-v20201202-2ch-v20200809-3ch-v20200603-4ch-v20201122.csv', index=False)
...
subset += [f'LAEDV_poisson_{label}' for label in labels]
subset += [f'LAEF_poisson_{label}' for label in labels]
subset += ['LAESV', 'LAEDV', 'LAEF']

df_inuse = df_all_petersen
df_inuse['LAESV'] = df_inuse['LA_Biplan_vol_min']
df_inuse['LAEDV'] = df_inuse['LA_Biplan_vol_max']
df_inuse['LAEF'] = 100.0*(df_inuse['LAEDV'] - df_inuse['LAESV']) / df_inuse['LAEDV'] 
df_inuse = df_inuse.dropna(subset=subset)
...
for feat in subset:
    ...
    df_inuse = df_inuse[df_inuse[feat] > 1]
f, ax = plt.subplots(3, (len(subset)-3)//3)
f.set_size_inches((len(subset)-3)//3*2.5, 6)

for i, (meas, extent) in enumerate(zip(['LAESV', 'LAEDV', 'LAEF'], [100, 200, 100])):
...
plt.tight_layout()
f.savefig('LA_petersen_all_20201211.png', dpi=500)
# %%
import scipy.stats
import matplotlib.pyplot as plt
