...
for chamber in chambers:
    ...
    for t in range(MRI_FRAMES):
        results[-1][f'{chamber}_sphere_{t}'] = [-1]*2*(end-start)
        # if 'LA' in chamber:
    ...
    try:
        ...
        for ii, instance in enumerate(instances):
            ...
            for channel, chamber, result in zip(channels, chambers, results):
                ...
                atria, volumes = annotation_to_poisson(annot_datasets, channel, views, annot_time_format_string, range(MRI_FRAMES), return_sphere=True)
                poisson_chambers.append(atria)
                poisson_volumes.append(volumes)
                for t, poisson_volume in enumerate(poisson_volumes[-1]):
                    result[f'{chamber}_sphere_{t}'][(i-start)*2+ii] = poisson_volume/1000.0
                ...
                for t, atrium in enumerate(poisson_chambers[-1]):
                    ...
                    to_xdmf(atrium, f'/home/pdiachil/projects/la_sphere/sphere_{version}_{chamber}_{sample_id}_{instance}', append=append, append_time=t, write_footer=write_footer)

            # # Extract major axes
            # la_hd5_file = f'/home/pdiachil/projects/la_sphere/poisson_{version}_LA_{sample_id}_{instance}.hd5'
            # lv_hd5_file = f'/home/pdiachil/projects/la_sphere/poisson_{version}_LV_{sample_id}_{instance}.hd5'
...
for chamber, result in zip(chambers, results):
    ...
    results_df.to_csv(f'{chamber}_processed_sphere_{version}_{start}_{end}.csv', index=False)
