...
df_pet.to_csv('/home/pdiachil/ml/notebooks/mri/petersen_atria_boundary.csv')
...
for df, label in zip([df_pet], ['poisson']):
    ...
    ell_max = cleaned_dfs[-1].values.max(axis=1)
    ell_min = cleaned_dfs[-1].values.min(axis=1)
    ...
    for nidxs in [1000, 2000, 3000, 4000]:
        ...
        plt.tight_layout()
        f.savefig(f'/home/pdiachil/ml/notebooks/mri/{label}_vol_boundary_{nidxs}.png', dpi=500)
