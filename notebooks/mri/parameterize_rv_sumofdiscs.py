...
from parameterize_segmentation import annotation_to_poisson, annotation_to_discs
...
hd5s = glob.glob('/mnt/disks/segmented-sax-v20201116-lax-v20201122-petersen/2020-11-22/*.hd5')
...
# start = 4
# end = start+1
version='sax_v20201116_lax_v20201122'
...
for chamber in chambers:
    ...
    results[-1]['width'] = [-1]*(end-start)
    results[-1]['height'] = [-1]*(end-start)
    results[-1]['depth'] = [-1]*(end-start)
    for t in range(MRI_FRAMES):
        results[-1][f'{chamber}_discs_{t}'] = [-1]*(end-start)
        ...
        with h5py.File(hd5) as ff_trad:
            for iv, view in enumerate(views):
                if view_format_string.format(view=view) not in ff_trad['ukb_cardiac_mri']:
                    ...
                    break
                height = ff_trad['mri_pixel_width_cine_segmented_sax_b1'][()]
                width = ff_trad['mri_pixel_width_cine_segmented_sax_b1'][()]
            ...
            point0 = ns.vtk_to_numpy(annot_datasets[-1].GetPoints().GetData())[0]
            point1 = ns.vtk_to_numpy(annot_datasets[-2].GetPoints().GetData())[0]
            depth = np.linalg.norm(point0 - point1)
            logging.info(f'Slice thickness is {depth}')
      # except:
      #     pass

        for channel, chamber, result in zip(channels, chambers, results):
            result['sample_id'][i-start] = sample_id
            result['height'][i-start] = height
            result['width'][i-start] = width
            result['depth'][i-start] = depth
            pixels = annotation_to_discs(
                annot_datasets, channel, views,
                annot_time_format_string,
                range(MRI_FRAMES), 0,
            )
            volumes = pixels * width * height * depth
            for t, volume in enumerate(volumes):
                result[f'{chamber}_discs_{t}'][i-start] = volume/1000.0
...
for chamber, result in zip(chambers, results):
    ...
    results_df.to_csv(f'{chamber}_discs_{version}_{start}_{end}.csv', index=False)
end_time = time.time()
