...
def find_learning_rate(
    ...
    """
    beta = 0.98  # parameter for smoothing of loss. Larger is more smooth.
    ...
    if output_folder:
        pd.DataFrame(
            {
                "loss": losses,
                "learning_rate": lrs[: len(losses)],
                "smoothed_loss": smoothed_losses,
                "picked_lr": best_lr,
            },
        ).to_csv(os.path.join(output_folder, "find_learning_rate.csv"), index=False)
        plot_find_learning_rate(
            learning_rates=lrs[: len(losses)],
            losses=losses,
            smoothed_losses=smoothed_losses,
            picked_learning_rate=best_lr,
...
def _choose_best_learning_rate(smoothed_loss: np.ndarray, lr: np.ndarray) -> float:
    ...
    for i in range(burn_in, len(smoothed_loss)):
        ...
        delta = (
            smoothed_loss[i - mean_width - 1 : i - 1].mean()
            - smoothed_loss[i - mean_width : i].mean()
        )  # positive is good, means loss decreased a lot
        if delta > best_delta:
            best_delta = delta
            best_lr = lr[i]
