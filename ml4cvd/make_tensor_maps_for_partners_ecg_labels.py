...
def _write_tmap_to_py(write_imports, py_file, label_maps, channel_maps, keys_in_hd5):
    ...
    for label in label_maps:
        ...
        for key in keys_in_hd5:
            py_file.write(f"TMAPS['{PREFIX}_{key}_{label}'] = TensorMap('{PREFIX}_{key}_{label}', path_prefix='{TENSOR_PATH_PREFIX}', interpretation=Interpretation.CATEGORICAL, channel_map={cm}, tensor_from_file={TENSOR_FUNC_NAME}(keys='{key}', dict_of_list = {label_maps[label]})) \n\n")
