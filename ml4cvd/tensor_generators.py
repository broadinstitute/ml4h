...
class TensorGenerator:
    def __init__(
        ...
        if weights is None:
            ...
            self.path_iters = [
                _WeightedPaths(p, weights) for p in worker_paths
            ]
    ...
    def _init_workers(self):
        ...
        for i, (path_iter, iter_len) in enumerate(
            ...
            if not self.run_on_main_thread:
                process = Process(
                    target=worker_instance.multiprocessing_worker,
                    name=name,
                    args=(),
                )
                process.start()
...
class TensorMapArrayCache:
    ...
    def __init__(
        ...
        self.data = {}
        self.row_size = sum(
            np.zeros(tm.shape, dtype=np.float32).nbytes
            for tm in set(input_tms + output_tms)
        )
        self.nrows = (
            min(int(max_size / self.row_size), max_rows)
            if self.row_size
            else 0
        )
...
class _MultiModalMultiTaskWorker:
    def __init__(
        ...
        self.paths_in_batch = []
        self.in_batch = {
            tm.input_name(): np.zeros((batch_size,) + tm.shape)
            for tm in input_maps
        }
        self.out_batch = {
            tm.output_name(): np.zeros((batch_size,) + tm.shape)
...
def big_batch_from_minibatch_generator(
    ...
    for key, batch_array in chain(
        first_batch[BATCH_INPUT_INDEX].items(),
        first_batch[BATCH_OUTPUT_INDEX].items(),
    ):
...
def _sample_csv_to_set(
    sample_csv: Optional[str] = None, mrn_column_name: Optional[str] = None,
) -> Union[None, Set[str]]:
    ...
    if mrn_column_name is None:
        ...
        possible_mrn_col_names = {"mrn", "patient_id", "medrecn"}
...
def test_train_valid_tensor_generators(
    ...
    **kwargs,
    ...
    if len(balance_csvs) > 0:
        ...
        weights = [
            1.0 / (len(balance_csvs) + 1) for _ in range(len(balance_csvs) + 1)
        ]
    else:
...
def _mixup_batch(
    ...
    mixed_ins = {
        k: np.zeros((half_batch,) + in_batch[k].shape[1:]) for k in in_batch
    }
    mixed_outs = {
...
def _make_batch_siamese(
    ...
    siamese_in = {
        k + "_left": np.zeros((half_batch,) + in_batch[k].shape[1:])
        for k in in_batch
    }
    siamese_in.update(
        {
