            ...
            try:
                with h5py.File(tp, 'r') as hd5:
                    ...
                    for tm in input_maps:
                        in_batch[tm.input_name()][stats['batch_index']] = tm.validator(tm.tensor_from_file(tm, hd5, dependents))
                    
                    for tm in output_maps:
                        if tm in dependents:
                            out_batch[tm.output_name()][stats['batch_index']] = dependents[tm]
                        else:
                            out_batch[tm.output_name()][stats['batch_index']] = tm.validator(tm.tensor_from_file(tm, hd5))
