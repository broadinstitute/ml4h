            ...
            if not write_pngs:
                ...
                if tm.is_categorical() and len(tm.shape) == 3:
                    ...
                    for yi in range(y.shape[0]):
                        for j in range(y.shape[3]):
                            truth = np.argmax(
                                test_labels[tm.output_name()][yi, :, :, j, :],
                                axis=-1,
                            )
        ...
        with h5py.File(test_paths[i], "r") as hd5:
            ...
            if "read_" in language_map.name:
                caption = decompress_data(
                    data_compressed=hd5[tm.name][()],
                    dtype=hd5[tm.name].attrs["dtype"],
                )
            else:
                caption = str(
...
def _tabulate_correlations(
    ...
    for field1, field2 in field_pairs:
        if (
            field1 not in nan_counter.keys()
            ...
            if num_common_samples >= min_samples:
                ...
                if len(field1_values) == len(field2_values):
                    if (
                        len(set(field1_values)) == 1
                        or len(set(field2_values)) == 1
                    ):
                        logging.debug(
...
def _collect_continuous_stats_from_tensor_file(
    ...
    def _field_meaning_to_values_dict(_, obj):
        if _is_continuous_valid_scalar_hd5_dataset(obj):
            ...
            if (
                len(dataset_name_parts) == 4
            ):  # e.g. /continuous/1488_Tea-intake_0_0
                ...
                if instance in instances:
                    if max_arr_idx is None or (
                        max_arr_idx is not None
                        and int(array_idx) <= max_arr_idx
                    ):
                        stats[
...
def _tensors_to_df(args):
    ...
    paths = [
        ...
        for path in gen.path_iters[0].paths
    ]
    tot = len(paths)
    ...
    list_of_tensor_dicts = [
        dotd[i]
        for dotd in list_of_dicts_of_dicts
        for i in dotd
        if dotd is not None
    ]
        ...
        for df_cur, df_str in zip([df, df.dropna()], ["union", "intersect"]):
            ...
            if df_cur.empty:
                ...
                for tm in [
                    tm
                    for tm in tmaps
                    if tm.interpretation is Interpretation.CONTINUOUS
                ]:
                    if tm.channel_map:
                        for cm in tm.channel_map:
                            stats = dict()
...
def _report_cross_reference(args, cross_reference_df, title):
    ...
    if args.reference_label in cross_reference_df:
        ...
        fpath = os.path.join(
            args.output_folder,
            args.id,
            f'distribution_{args.reference_label.replace(" ", "_")}_{title}.csv',
        )
        df_out.to_csv(fpath)
        logging.info(
...
def cross_reference(args):
    ...
    use_time = (
        src_time is not None and len(ref_start) != 0 and len(ref_end) != 0
    )
    if use_time:
    ...
    cohort_counts[
        f'{src_name} in {ref_name} (unique {" + ".join(src_cols)})'
    ] = len(cross_reference_df.drop_duplicates(subset=src_cols))
    cohort_counts[
        f'{src_name} in {ref_name} (unique {" + ".join(src_join)})'
    ] = len(cross_reference_df.drop_duplicates(subset=src_join))
    cohort_counts[
        f'{ref_name} in {src_name} (unique {" + ".join(ref_cols)})'
    ] = len(cross_reference_df.drop_duplicates(subset=ref_cols))
    cohort_counts[
        f'{ref_name} in {src_name} (unique {" + ".join(ref_join)})'
    ] = len(cross_reference_df.drop_duplicates(subset=ref_join))
    ...
    if use_time:
        cross_reference_df = cross_reference_df[
            (cross_reference_df[ref_start] <= cross_reference_df[src_time])
            & (cross_reference_df[ref_end] >= cross_reference_df[src_time])
        ]
        ...
        cohort_counts[
            f"{src_name} {time_description} (total - {src_name} may be duplicated if valid for multiple {ref_name})"
        ] = len(cross_reference_df)
        cohort_counts[
            f'{src_name} {time_description} (unique {" + ".join(src_cols)})'
        ] = len(cross_reference_df.drop_duplicates(subset=src_cols))
        ...
        cohort_counts[
            f"Most recent {src_name} in {ref_name} {time_description} (total - {src_name} may be duplicated if valid for multiple {ref_name})"
        ] = len(cross_reference_df)
        cohort_counts[
            f'Most recent {src_name} in {ref_name} {time_description} (unique {" + ".join(src_cols)})'
        ] = len(cross_reference_df.drop_duplicates(subset=src_cols))
