...
# Imports: first party
from ml4cvd.plots import plot_histograms, plot_value_counter
...
MRI_CARDIAC_SERIES = [
    ...
    "cine_segmented_sax_b1",
    "cine_segmented_sax_b2",
    "cine_segmented_sax_b3",
    "cine_segmented_sax_b4",
    "cine_segmented_sax_b5",
    "cine_segmented_sax_b6",
    "cine_segmented_sax_b7",
    "cine_segmented_sax_b8",
    "cine_segmented_sax_b9",
    "cine_segmented_sax_b10",
    "cine_segmented_sax_b11",
    "cine_segmented_sax_b12",
    "cine_segmented_sax_b13",
...
MRI_LIVER_SERIES_12BIT = [
    ...
    "shmolli_192i_12bit",
    "shmolli_192i_liver_12bit",
]
MRI_LIVER_IDEAL_PROTOCOL = [
    "lms_ideal_optimised_low_flip_6dyn",
    "lms_ideal_optimised_low_flip_6dyn_12bit",
]
...
def write_tensors(
    ...
    for sample_id in sorted(sample_ids):
        ...
        try:
            with h5py.File(tp, "w") as hd5:
                _write_tensors_from_zipped_dicoms(
                    zoom_x,
                    zoom_y,
                    zoom_width,
                    zoom_height,
                ...
                _write_tensors_from_zipped_niftis(
                    zip_folder, mri_field_ids, hd5, sample_id, stats,
                )
                _write_tensors_from_xml(
                    xml_field_ids,
                    xml_folder,
                    hd5,
                    sample_id,
            ...
            logging.exception(
                "Encountered AttributeError trying to write a UKBB tensor at path:{}".format(
                    tp,
                ),
            )
            ...
            logging.exception(
                "Encountered ValueError trying to write a UKBB tensor at path:{}".format(
                    tp,
                ),
            )
            logging.info("Deleting attempted tensor at path:{}".format(tp))
            ...
            logging.exception(
                "Encountered RuntimeError trying to write a UKBB tensor at path:{}".format(
                    tp,
                ),
            )
            logging.info("Deleting attempted tensor at path:{}".format(tp))
            ...
            logging.exception(
                "Encountered IndexError trying to write a UKBB tensor at path:{}".format(
                    tp,
                ),
            )
            logging.info("Deleting attempted tensor at path:{}".format(tp))
            ...
            logging.exception(
                "Encountered OSError trying to write a UKBB tensor at path:{}".format(
                    tp,
                ),
            )
            logging.info("Deleting attempted tensor at path:{}".format(tp))
    ...
    _dicts_and_plots_from_tensorization(
        a_id, output_folder, min_values_to_print, write_pngs, continuous_stats, stats,
    )
...
def write_tensors_from_dicom_pngs(
    ...
    dicom_header="dicom_file",
    instance_header="instance_number",
            ...
            with h5py.File(tensor_file, "a") as hd5:
                ...
                if tp in hd5:
                    ...
                    create_tensor_in_hd5(
                        hd5, path_prefix, tensor_name, full_tensor, stats,
                    )
                    stats["created"] += 1
...
def write_tensors_from_ecg_pngs(
    tensors,
    png_path,
    min_sample_id,
    max_sample_id,
        ...
        with h5py.File(tensor_file, "a") as hd5:
            ...
            if tp in hd5:
                ...
                create_tensor_in_hd5(
                    ...
                    ecg_1d_segmentation,
                    stats,
                    attributes={"offset_seconds": offset_seconds},
                )
                stats["created"] += 1
    ...
    with open(volume_csv, "r") as volumes:
        ...
        for row in lol[1:]:
            ...
            if min_sample_id <= sample_id <= max_sample_id:
                nested_dictionary[sample_id] = {
                    fields[i].strip().lower(): row[i + 1] for i in range(len(fields))
                }

    with open(lv_mass_csv, "r") as lvm:
        lol = list(csv.reader(lvm, delimiter="\t"))
        logging.info(
            "CSV of returned MRI mass, etc, header:{}".format(list(enumerate(lol[0]))),
        )
        for row in lol[1:]:
            # column 0 is the original app's sample ID. Column 1 is app 7089's sample ID.
            sample_id = int(row[1])
            if (
                min_sample_id <= sample_id <= max_sample_id
                and len(row[13]) > 0
                and row[13] != "NA"
            ):
...
def _sample_has_dicom_mris(zip_folder, sample_id) -> bool:
    ...
    return any(
        [
            os.path.exists(os.path.join(zip_folder, f"{sample_str}_{mri_f}_2_0.zip"))
            for mri_f in DICOM_MRI_FIELDS
        ],
    )
...
def _sample_has_nifti_mris(zip_folder, sample_id) -> bool:
    ...
    return any(
        [
            os.path.exists(os.path.join(zip_folder, f"{sample_str}_{mri_f}_2_0.zip"))
            for mri_f in MRI_NIFTI_FIELD_ID_TO_ROOT
        ],
    )
...
def _dicts_and_plots_from_tensorization(
    ...
    if write_pngs:
        plot_value_counter(
            ...
            value_counter,
            a_id + "_v_count",
            os.path.join(output_folder, a_id),
        )
...
def _write_tensors_from_zipped_dicoms(
    ...
    for mri_field in set(mri_field_ids).intersection(DICOM_MRI_FIELDS):
        ...
        for zipped in mris:
            logging.info(
                "Got zipped dicoms for sample: {} with MRI field: {}".format(
                    sample_id, mri_field,
                ),
            )
...
def _write_tensors_from_zipped_niftis(
    zip_folder: str,
    mri_field_ids: List[str],
    hd5: h5py.File,
    sample_id: str,
    ...
    for mri_field in set(mri_field_ids).intersection(
        set(MRI_NIFTI_FIELD_ID_TO_ROOT.keys()),
    ):
...
def _write_tensors_from_dicoms(
    ...
    for dicom in os.listdir(dicom_folder):
        ...
        if (
            series + "_12bit" in MRI_LIVER_SERIES_12BIT
            and d.LargestImagePixelValue > 2048
        ):
    ...
    for v in views:
        ...
        if v == MRI_TO_SEGMENT:
            _tensorize_short_axis_segmented_cardiac_mri(
                views[v],
                v,
                zoom_x,
                zoom_y,
            ...
            for slicer in views[v]:
                ...
                if v in MRI_LIVER_IDEAL_PROTOCOL:
                    slice_index = _slice_index_from_ideal_protocol(
                        slicer, min_ideal_series,
                    )
...
def _tensorize_short_axis_segmented_cardiac_mri(
    ...
    for slicer in slices:
        ...
        if _has_overlay(slicer):
            ...
            try:
                ...
                logging.exception(
                    f"Got key error trying to make anatomical mask, skipping.",
                )
                continue

            _save_pixel_dimensions_if_missing(slicer, series, hd5)
            _save_slice_thickness_if_missing(slicer, series, hd5)
            _save_series_orientation_and_position_if_missing(
                slicer, series, hd5, str(slicer.InstanceNumber),
            )
            ...
            _save_series_orientation_and_position_if_missing(
                slicer, series_segmented, hd5, str(slicer.InstanceNumber),
            )

            cur_angle = (
                slicer.InstanceNumber - 1
            ) // MRI_FRAMES  # dicom InstanceNumber is 1-based
            full_slice[:] = slicer.pixel_array.astype(np.float32)
            create_tensor_in_hd5(
                hd5,
                mri_group,
                f"{series}{HD5_GROUP_CHAR}{slicer.InstanceNumber}",
                full_slice,
                stats,
                mri_date,
            )
            create_tensor_in_hd5(
                hd5,
                mri_group,
                f"{series_zoom_segmented}{HD5_GROUP_CHAR}{slicer.InstanceNumber}",
                mask,
                stats,
                mri_date,
            )

            zoom_slice = full_slice[
                zoom_x : zoom_x + zoom_width, zoom_y : zoom_y + zoom_height
            ]
            zoom_mask = mask[
                zoom_x : zoom_x + zoom_width, zoom_y : zoom_y + zoom_height
            ]
            create_tensor_in_hd5(
                hd5,
                mri_group,
                f"{series_zoom}{HD5_GROUP_CHAR}{slicer.InstanceNumber}",
                zoom_slice,
                stats,
                mri_date,
            )
            ...
            if (
                slicer.InstanceNumber - 1
            ) % MRI_FRAMES == 0:  # Diastole frame is always the first
    ...
    for angle in diastoles:
        ...
        create_tensor_in_hd5(
            hd5, mri_group, f"diastole_frame_b{angle}", full_slice, stats, mri_date,
        )
        create_tensor_in_hd5(
            hd5,
            mri_group,
            f"diastole_mask_b{angle}",
            diastoles_masks[angle],
            stats,
            mri_date,
        )
        if write_pngs:
            plt.imsave(
                tensors + "diastole_frame_b" + str(angle) + IMAGE_EXT, full_slice,
            )
            plt.imsave(tensors + "diastole_mask_b" + str(angle) + IMAGE_EXT, full_mask)

        full_slice = systoles[angle].pixel_array.astype(np.float32)
        create_tensor_in_hd5(
            hd5, mri_group, f"systole_frame_b{angle}", full_slice, stats, mri_date,
        )
        create_tensor_in_hd5(
            hd5,
            mri_group,
            f"systole_mask_b{angle}",
            systoles_masks[angle],
            stats,
            mri_date,
        )
        if write_pngs:
...
def _tensorize_brain_mri(
    ...
    mri_data1 = np.zeros(
        (slices[0].Rows, slices[0].Columns, len(slices) // 2), dtype=np.float32,
    )
    mri_data2 = np.zeros(
        (slices[0].Rows, slices[0].Columns, len(slices) // 2), dtype=np.float32,
    )
...
def _save_pixel_dimensions_if_missing(slicer, series, hd5):
    if (
        MRI_PIXEL_WIDTH + "_" + series not in hd5
        and series
        in MRI_BRAIN_SERIES
        + MRI_CARDIAC_SERIES
        + MRI_CARDIAC_SERIES_SEGMENTED
        + MRI_LIVER_SERIES
        + MRI_LIVER_SERIES_12BIT
    ):
        hd5.create_dataset(
            MRI_PIXEL_WIDTH + "_" + series, data=float(slicer.PixelSpacing[0]),
        )
    if (
        MRI_PIXEL_HEIGHT + "_" + series not in hd5
        and series
        in MRI_BRAIN_SERIES
        + MRI_CARDIAC_SERIES
        + MRI_CARDIAC_SERIES_SEGMENTED
        ...
        hd5.create_dataset(
            MRI_PIXEL_HEIGHT + "_" + series, data=float(slicer.PixelSpacing[1]),
        )


def _save_slice_thickness_if_missing(slicer, series, hd5):
    if (
        MRI_SLICE_THICKNESS + "_" + series not in hd5
        and series
        in MRI_BRAIN_SERIES
        + MRI_CARDIAC_SERIES
        + MRI_CARDIAC_SERIES_SEGMENTED
        ...
        hd5.create_dataset(
            MRI_SLICE_THICKNESS + "_" + series, data=float(slicer.SliceThickness),
        )


def _save_series_orientation_and_position_if_missing(
    slicer, series, hd5, instance=None,
):
    ...
    if (
        orientation_ds_name not in hd5
        and series
        in MRI_BRAIN_SERIES
        + MRI_CARDIAC_SERIES
        + MRI_CARDIAC_SERIES_SEGMENTED
    ...
    if (
        ...
        hd5.create_dataset(
            position_ds_name, data=[float(x) for x in slicer.ImagePositionPatient],
        )
...
def _get_overlay_from_dicom(d, debug=False) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if overlay_frames == 1:
        ...
        if (
            ventricle_pixels == 0 and myocardium_pixels > MRI_MAX_MYOCARDIUM
        ):  # try to rescue small ventricles
            erode_structure = _unit_disk(small_radius * 1.5)
            anatomical_mask = anatomical_mask - binary_erosion(
                m1, erode_structure,
            ).astype(np.int)
            ventricle_pixels = np.count_nonzero(
                anatomical_mask == MRI_SEGMENTED_CHANNEL_MAP["ventricle"],
...
def _write_tensors_from_xml(
    xml_field_ids, xml_folder, hd5, sample_id, write_pngs, stats, continuous_stats,
) -> None:
    for xml_field in xml_field_ids:
        xmlp = xml_folder + str(sample_id) + "_" + xml_field + "*.xml"
        ecgs = glob.glob(xmlp)
        if xml_field == ECG_REST_FIELD:
            _write_ecg_rest_tensors(
                ecgs, xml_field, hd5, sample_id, write_pngs, stats, continuous_stats,
            )
    ...
    for ecg in ecgs:
        ...
        for d in root.findall("./Interpretation/Diagnosis/DiagnosisText"):
            if "QRS Complexes:" in d.text:
                ...
                create_tensor_in_hd5(
                    hd5, rest_group, "QRSComplexes", qrs, stats, date=ecg_date,
                )
            elif (
                "---" in d.text
                or "Arrhythmia results of the full-disclosure ECG" in d.text
            ):
                continue
            else:
        ...
        for c in root.findall("./StripData/WaveformData"):
            ...
            create_tensor_in_hd5(
                hd5, rest_group, dataset_name, lead_data, stats, date=ecg_date,
            )
                ...
                if child.tag == "MedianSamples":
                    for median_c in child:
                        if median_c.tag == "WaveformData":
                            median_wave = list(
                                map(float, median_c.text.strip().split(",")),
                            )
...
def _write_ecg_bike_tensors(ecgs, xml_field, hd5, sample_id, stats):
    for ecg in ecgs:
        ...
        for median_waves in root.findall("./MedianData/Median/WaveformData"):
            median_ecgs[median_waves.attrib["lead"]].extend(
                list(map(float, median_waves.text.strip().split(","))),
            )
        if len(median_ecgs) > 0:
            ...
            for lead in median_ecgs:
                ...
                median_np[:median_idx, ECG_BIKE_LEADS[lead]] = median_ecgs[lead][
                    :median_idx
                ]
        ...
        for strip_waves in root.findall("./StripData/Strip/WaveformData"):
            ...
            strip_np[
                :strip_idx, ECG_BIKE_LEADS[strip_waves.attrib["lead"]]
            ] = strip_list[:strip_idx]
        if counter > 0:
            ...
            for full_line in re.split("\n|\t", full_d.text):
                for sample in re.split(",", full_line):
                    if sample == "":
                        continue
                    lead = (
                        count % (ECG_BIKE_NUM_LEADS * ECG_BIKE_SAMPLE_RATE)
                    ) // ECG_BIKE_SAMPLE_RATE
        ...
        trends = {
            trend: np.full(len(trend_entries), np.nan)
            for trend in trend_entry_fields
            + ["time", "PhaseTime", "PhaseTime", "PhaseName", "Artifact"]
        }
        trends.update(
            {
                trend: np.full((len(trend_entries), 3), np.nan)
                for trend in trend_lead_measurements
            },
        )
        ...
        for i, trend_entry in enumerate(trend_entries):
            ...
            for lead_field, lead in product(
                trend_lead_measurements, trend_entry.findall("LeadMeasurements"),
            ):
            ...
            trends["time"][i] = SECONDS_PER_MINUTE * int(
                trend_entry.find("EntryTime/Minute").text,
            ) + int(trend_entry.find("EntryTime/Second").text)
            trends["PhaseTime"][i] = SECONDS_PER_MINUTE * int(
                trend_entry.find("PhaseTime/Minute").text,
            ) + int(trend_entry.find("PhaseTime/Second").text)
            trends["PhaseName"][i] = phase_to_int[trend_entry.find("PhaseName").text]
            trends["Artifact"][i] = (
                float(trend_entry.find("Artifact").text.strip("%")) / 100
        ...
        for protocol in root.findall("./Protocol/Phase"):
            ...
            phase_duration = SECONDS_PER_MINUTE * int(
                protocol.find("PhaseDuration/Minute").text,
            ) + int(protocol.find("PhaseDuration/Second").text)
            phase_durations[phase_name] = phase_duration
            write_to_hd5(
                name=f"{str.lower(phase_name)}_duration", value=[phase_duration],
            )

        # HR stats
        max_hr = _xml_path_to_float(root, "./ExerciseMeasurements/MaxHeartRate")
        resting_hr = _xml_path_to_float(
            root, "./ExerciseMeasurements/RestingStats/RestHR",
        )
...
def _write_tensors_from_niftis(
    folder: str, hd5: h5py.File, field_id: str, stats: Counter,
):
    niftis = glob.glob(
        os.path.join(folder, MRI_NIFTI_FIELD_ID_TO_ROOT[field_id], "**/*nii.gz"),
        recursive=True,
    )
    logging.info(
    ...
    for nifti in niftis:  # iterate through all nii.gz files and add them to the hd5
        ...
        if (
            MRI_NIFTI_FIELD_ID_TO_ROOT[field_id] == "SWI"
            and nifti_array.shape[-1] == MRI_SWI_SLICES_TO_AXIS_SHIFT
            and nifti_array.shape[0] > nifti_array.shape[1]
        ):
...
def _get_disease_censor_dates(
    disease2tsv, min_sample_id=0, max_sample_id=9e9,
) -> Dict[str, Dict[int, List[datetime.datetime]]]:
...
def append_fields_from_csv(tensors, csv_file, group, delimiter):
    ...
    categorical_channel_maps = {
        MRI_ANNOTATION_NAME: MRI_ANNOTATION_CHANNEL_MAP,
    }  # str: dict
                            ...
                            for cm_name in categorical_channel_maps:
                                if value in categorical_channel_maps[cm_name]:
                                    ...
                                    if cm_name in hd5[group]:
                                        data = hd5[hd5_key]
                                        data[0] = categorical_channel_maps[cm_name][
                                            value
                                        ]
                                        stats["updated"] += 1
                                    else:
...
def _ukbb_stats(
    run_id, output_folder, phenos_folder, volume_csv, icd_csv, app_csv, zip_folder,
) -> None:
    stats = Counter()
    ...
    for row in lol[1:]:
        sample_id = int(row[0])
        if (
            os.path.exists(zip_folder + row[0] + "_20209_2_0.zip")
            ...
            if icds[disease][sample_id] != "NA" and sample_id in status[disease]:
                if status[disease][sample_id] != int(icds[disease][sample_id]):
                    stats[
                        disease
                        + "_icd_disparity_"
                        + icds[disease][sample_id]
                        + "_censor_"
                        + str(status[disease][sample_id])
                    ] += 1
                else:
    ...
    for i, d in enumerate(disease_dates):
        ...
        logging.info(
            "Try to plot figure at:{} with {} dates.".format(
                figure_path, len(disease_dates[d]),
            ),
        )
...
def _date_from_dicom(d) -> str:
    return (
        d.AcquisitionDate[0:4]
        + CONCAT_CHAR
        + d.AcquisitionDate[4:6]
        + CONCAT_CHAR
        + d.AcquisitionDate[6:]
    )
...
def _prune_sample(
    ...
    if sample_id > max_sample_id:
        return True
    if len(mri_field_ids) > 0 and not (
        _sample_has_dicom_mris(zip_folder, sample_id)
        or _sample_has_nifti_mris(zip_folder, sample_id)
    ):
        return True
    if len(xml_field_ids) > 0 and not _sample_has_ecgs(
        xml_folder, xml_field_ids, sample_id,
    ):
        return True
