...
def survival_likelihood_loss(n_intervals):
    ...
    def loss(y_true, y_pred):
        ...
        """
        survival_likelihood = 1. + (y_true[:, 0:n_intervals] * (y_pred[:, 0:n_intervals]-1.))  # Loss only for intervals that were survived
        failure_likelihood = 1. - (y_true[:, 0:n_intervals] * y_pred[:, 0:n_intervals])  # Loss only for individuals who failed
