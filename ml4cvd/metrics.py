...
def survival_likelihood_loss(n_intervals):
    ...
    def loss(y_true, y_pred):
        ...
        failure_likelihood = y_true[:, n_intervals:2 * n_intervals] * (1-y_true[:, 0:n_intervals]) * (1-y_pred[:, 0:n_intervals])  # Loss only for individuals who failed
        failure_likelihood += 1. - y_true[:, n_intervals:2 * n_intervals]
