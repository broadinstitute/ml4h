...
def survival_likelihood_loss(n_intervals):
    ...
    def loss(y_true, y_pred):
        ...
        """
        likelihood = K.maximum(y_true[:, n_intervals:2 * n_intervals], 0) * (1.-y_true[:, 0:n_intervals]) * (1.-y_pred[:, 0:n_intervals])  # Loss only for individuals who failed
        likelihood += y_true[:, 0:n_intervals] * y_pred[:, 0:n_intervals]  # Loss for intervals that were survived
        return K.sum(-K.log(K.clip(likelihood, K.epsilon(), None)), axis=-1)  # return -log likelihood
