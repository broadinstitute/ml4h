...
LESS_THAN_CODES = (
    "('Less than a year', 'Less than once a week', 'Less than one mile', 'Less than an"
    " hour a day', 'Less than one a day', 'Less than one', 'Less than once a year',"
    " 'Less than 1 year ago', 'Less than a year ago', 'Less than one year', 'Less than"
...
def _get_tensor_map_file_imports() -> str:
    return (
        ...
        " ml4cvd.tensor_maps_by_hand import TMAPS\nfrom ml4cvd.metrics import"
        " weighted_crossentropy\nfrom ml4cvd.tensor_from_file import"
        " prevalent_incident_tensor\nfrom ml4cvd.TensorMap import TensorMap,"
        " Interpretation, make_range_validator\n\n\n"
    )
...
def _write_disease_tensor_maps(phenos_folder: str, f: TextIO) -> None:
    ...
    for d in sorted(list(disease2tsv.keys())):
        ...
        f.write(
            f"TMAPS['{d}'] = TensorMap('{d}', Interpretation.CATEGORICAL,"
            " storage_type=StorageType.CATEGORICAL_FLAG, path_prefix='categorical',"
            f" channel_map={{'no_{d}':0, '{d}':1}}, loss=weighted_crossentropy([1.0,"
            f" {factor}], '{d}'))\n",
        )
...
def _write_disease_tensor_maps_incident_prevalent(
    ...
    for disease in sorted(list(disease2tsv.keys())):
        ...
        f.write(
            f"TMAPS['{disease}_prevalent_incident'] = TensorMap('{disease}',"
            " Interpretation.CATEGORICAL,  storage_type=StorageType.CATEGORICAL_FLAG,"
            " path_prefix='categorical',"
            " tensor_from_file=prevalent_incident_tensor('dates/enroll_date',"
            f" 'dates/{disease}_date'), channel_map={{'no_{disease}':0,"
            f" 'prevalent_{disease}':1, 'incident_{disease}':2}},"
            f" loss=weighted_crossentropy([1.0, {factor_p}, {factor_i}],"
            f" '{disease}_prevalent_incident'))\n",
        )
...
def _write_phecode_tensor_maps(f: TextIO, phecode_csv, db_client: DatabaseClient):
    ...
    query = (
        f"select disease, count(disease) as total from"
        f" `broad-ml4cvd.ukbb7089_201904.phecodes_nonzero` GROUP BY disease"
    )
    ...
    for k, p in sorted(phecode2phenos.items(), key=operator.itemgetter(1)):
        if k in phecode2counts:
            ...
            f.write(
                f"TMAPS['{p}_phe'] = TensorMap('{k}', Interpretation.CATEGORICAL,"
                f" channel_map={{'no_{p}':0, '{p}':1}}, path_prefix='categorical',"
                " storage_type=StorageType.CATEGORICAL_FLAG,"
                f" loss=weighted_crossentropy([1.0, {factor}],"
                f" '{k.replace('.', '_')}'))\n",
            )

    query = (
        f"select disease, count(disease) as total from"
        f" `broad-ml4cvd.ukbb7089_201904.phecodes_nonzero` WHERE prevalent_disease=1"
        f" GROUP BY disease"
    )
    count_result = db_client.execute(query)
    phecode2prevalent = {}
    ...
    query = (
        f"select disease, count(disease) as total from"
        f" `broad-ml4cvd.ukbb7089_201904.phecodes_nonzero` WHERE incident_disease=1"
        f" GROUP BY disease"
    )
    count_result = db_client.execute(query)
    phecode2incident = {}
    ...
    for k, p in sorted(phecode2phenos.items(), key=operator.itemgetter(1)):
        if k in phecode2incident and k in phecode2prevalent:
            ...
            f.write(
                f"TMAPS['{p}_phe_pi'] = TensorMap('{k}',  Interpretation.CATEGORICAL, "
                " storage_type=StorageType.CATEGORICAL_FLAG,"
                " path_prefix='categorical',"
                " tensor_from_file=prevalent_incident_tensor('dates/enroll_date',"
                f" 'dates/{k}_date'), channel_map={{'no_{p}':0, '{p}_prevalent':1,"
                f" '{p}_incident':2}}, loss=weighted_crossentropy([1.0, {factor_p},"
                f" {factor_i}], '{p}_pi'))\n",
            )
...
def _write_continuous_tensor_maps(f: TextIO, db_client: DatabaseClient):
    ...
    for row in field_data_for_tensor_maps:
        ...
        channel_map += "}"
        f.write(
            f"TMAPS['{row.FieldID}_{row.instance}'] = TensorMap('{name}{JOIN_CHAR}{i}',"
            " loss='logcosh', path_prefix='continuous', ",
        )
        f.write(
            f"normalization={{'mean': {row.mean}, 'std': {row.std}}},"
            f" annotation_units={row.max_array+1}, {channel_map})\n",
        )
