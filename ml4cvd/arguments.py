...
def parse_args():
    ...
    parser.add_argument(
        "--mode", default="mlp", help="What would you like to do?",
    )
    ...
    parser.add_argument(
        "--sample_weight",
        default=None,
        help="TensorMap key for sample weight in training.",
    )
    parser.add_argument(
        "--tensor_maps_in",
        default=[],
    ...
    parser.add_argument("--explore_export_errors",default=False,action="store_true",help="Export of error_type columns in the tensors csv generated by explore mode.")
    # Model Architecture Parameters
    parser.add_argument(
        "--x", default=256, type=int, help="x tensor resolution",
    )
    parser.add_argument(
        "--y", default=256, type=int, help="y tensor resolution",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--z", default=48, type=int, help="z tensor resolution",
    )
    parser.add_argument(
        "--t", default=48, type=int, help="Number of time slices",
    )
    parser.add_argument(
        "--mlp_concat",
        default=False,
...
def _get_tmap(name: str, needed_tensor_maps: List[str]) -> TensorMap:
    ...
    from ml4cvd.tensor_maps_partners_ecg_labels import (
        TMAPS as partners_label_tmaps,
    )
...
def _process_u_connect_args(
    ...
    for connect_pair in u_connect:
        ...
        tmap_in, tmap_out = (
            _get_tmap(tmap_key_in, []),
            _get_tmap(tmap_key_out, []),
        )
        ...
        if tmap_in.axes() < 2 or tmap_out.axes() < 2:
            raise TypeError(
                f"Cannot u_connect 1d TensorMaps ({tmap_in} {tmap_out}).",
            )
