...
def parse_args():
    ...
    parser.add_argument('--tensors_per_file', default=1, type=int, help='If there are multiple tensors per HD5, select how many tensors to use per HD5.')
    parser.add_argument('--which_tensors', default='NEWEST', choices=['NEWEST', 'OLDEST', 'RANDOM'], help='If there are multiple tensors per HD5, select which tensor(s) to use per HD5.')
...
def _process_args(args):
    ...
    if args.tensors_per_file < 1:
        raise ValueError(f'expected tensors_per_file to be > 0 but got {args.tensors_per_file}.')
