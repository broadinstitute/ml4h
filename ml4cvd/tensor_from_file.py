...
from ml4cvd.defines import (
    ...
    ECG_REST_AMP_LEADS,
    MRI_SLICE_THICKNESS,
    MRI_PATIENT_POSITION,
    ECG_REST_MEDIAN_LEADS,
    MRI_PATIENT_ORIENTATION,
    ECG_SEGMENTED_CHANNEL_MAP,
    MRI_SEGMENTED_CHANNEL_MAP,
    MRI_AO_SEGMENTED_CHANNEL_MAP,
    MRI_SAX_SEGMENTED_CHANNEL_MAP,
    MRI_LIVER_SEGMENTED_CHANNEL_MAP,
    MRI_LAX_3CH_SEGMENTED_CHANNEL_MAP,
    MRI_LAX_4CH_SEGMENTED_CHANNEL_MAP,
    StorageType,
)
from ml4cvd.metrics import cox_hazard_loss, weighted_crossentropy
    ...
    def _random_slice_tensor_from_file(tm: TensorMap, hd5: h5py.File, dependents=None):
        ...
        if dependent_key is not None:
            ...
            label_tensor = np.array(
                hd5[dependent_key][..., cur_slice], dtype=np.float32,
            )
...
def _slice_subset_tensor(
    ...
    def _slice_subset_tensor_from_file(tm: TensorMap, hd5: h5py.File, dependents=None):
        ...
        if dependent_key is not None:
            label_tensor = np.array(
                hd5[dependent_key][..., start:stop], dtype=np.float32,
            )
            dependents[tm.dependent_map] = to_categorical(
                label_tensor, tm.dependent_map.shape[-1],
            )
...
def _survival_tensor(
    start_date_key: str, day_window: int, incidence_only: bool = False,
):
...
def prevalent_incident_tensor(start_date_key, event_date_key):
    def _prevalent_incident_tensor_from_file(
        tm: TensorMap, hd5: h5py.File, dependents=None,
    ):
        index = 0
...
def _nan_to_mean(tensor, max_allowed_nan_fraction=0.2):
...
def _get_tensor_at_first_date(
    hd5: h5py.File, path_prefix: str, name: str, handle_nan=_fail_nan,
):
    """
...
def _pad_or_crop_array_to_shape(new_shape: Tuple, original: np.ndarray):
    ...
    slices = tuple(
        slice(min(original.shape[i], new_shape[i])) for i in range(len(original.shape))
    )
...
def _median_pretest(tm: TensorMap, hd5: h5py.File, dependents=None):
    ...
    tensor = np.abs(
        _get_tensor_at_first_date(hd5, tm.path_prefix, "float_array", tm.name),
    )
...
TMAPS["ecg-bike-healthy-max-hr"] = TensorMap(
    ...
    loss="logcosh",
    metrics=["mae"],
    normalization={"mean": 113.7, "std": 13.3},
    shape=(1,),
...
TMAPS["ecg-bike-med-pretest-hr"] = TensorMap(
    ...
    metrics=["mae"],
    shape=(1,),
    normalization={"mean": 70.0, "std": 11.0},
...
TMAPS["ecg-bike-med-pretest-stamp"] = TensorMap(
    ...
    metrics=["mae"],
    shape=(1,),
    normalization={"mean": 0.03, "std": 0.03},
    tensor_from_file=_median_pretest,
)
TMAPS["ecg-bike-med-pretest-jpoint"] = TensorMap(
    "trend_jpointamplitude",
    path_prefix="ecg_bike",
    loss="logcosh",
    metrics=["mae"],
    shape=(1,),
    normalization={"mean": 0.032, "std": 0.46},
    tensor_from_file=_median_pretest,
)
TMAPS["ecg-bike-med-pretest-stamp20"] = TensorMap(
    "trend_stamplitude20ms",
    path_prefix="ecg_bike",
    loss="logcosh",
    metrics=["mae"],
    shape=(1,),
...
TMAPS["ecg-bike-pretest"] = TensorMap(
    ...
    validator=no_nans,
    normalization={
        "mean": np.array([7, -7, 3.5])[np.newaxis],
        "std": np.array([31, 30, 16])[np.newaxis],
    },
...
TMAPS["ecg-bike-hrr-student"] = TensorMap(
    ...
    tensor_from_file=_build_tensor_from_file(
        "inference.tsv", "ecg-bike-hrr-sentinel_prediction",
    ),
)
TMAPS["ecg-bike-hr-achieved"] = TensorMap(
    ...
    metrics=["mae"],
    shape=(1,),
    normalization={"mean": 0.68, "std": 0.1},
...
TMAPS["enroll_diabetes2_hazard_incident"] = TensorMap(
    ...
    tensor_from_file=_survival_tensor(
        "dates/enroll_date", DAYS_IN_5_YEARS, incidence_only=True,
    ),
)
...
TMAPS["cox_mi"] = TensorMap(
    "myocardial_infarction",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date"),
)
TMAPS["cox_mi_incident"] = TensorMap(
    "myocardial_infarction",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date", incidence_only=True),
)
TMAPS["cox_hyp"] = TensorMap(
    "hypertension",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date"),
)
TMAPS["cox_hyp_incident"] = TensorMap(
    "hypertension",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date", incidence_only=True),
)
TMAPS["cox_cad"] = TensorMap(
    "coronary_artery_disease_soft",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date"),
)
TMAPS["cox_cad_incident"] = TensorMap(
    "coronary_artery_disease_soft",
    Interpretation.TIME_TO_EVENT,
    tensor_from_file=cox_tensor_from_file("dates/enroll_date", incidence_only=True),
)
...
def _make_ecg_rest(
    population_normalize: float = None,
    ...
    def ecg_rest_from_file(tm, hd5, dependents={}):
        ...
        if tm.dependent_map is not None:
            ...
            tensor = np.reshape(
                hd5[tm.path_prefix][lead_idx][: tensor.shape[0] * tensor.shape[1]],
                tensor.shape,
                order="F",
            )
                ...
                if k in tm.channel_map:
                    ...
                    if short_time_nperseg > 0 and short_time_noverlap > 0:
                        f, t, short_time_ft = scipy.signal.stft(
                            data,
                            nperseg=short_time_nperseg,
                            noverlap=short_time_noverlap,
                        )
...
TMAPS["ecg_rest_raw"] = TensorMap(
    ...
    tensor_from_file=_make_ecg_rest(population_normalize=2000.0),
...
TMAPS["ecg_rest_raw_roll"] = TensorMap(
    ...
    shape=(5000, 12),
    path_prefix="ukb_ecg_rest",
    tensor_from_file=_make_ecg_rest(population_normalize=2000.0, random_roll=True),
    channel_map=ECG_REST_LEADS,
    cacheable=False,
)
TMAPS["ecg_rest_raw_warp"] = TensorMap(
    ...
    shape=(5000, 12),
    path_prefix="ukb_ecg_rest",
    tensor_from_file=_make_ecg_rest(population_normalize=2000.0, warp=True),
    channel_map=ECG_REST_LEADS,
    cacheable=False,
)
TMAPS["ecg_rest_raw_warp_n_roll"] = TensorMap(
    ...
    shape=(5000, 12),
    path_prefix="ukb_ecg_rest",
    tensor_from_file=_make_ecg_rest(
        population_normalize=2000.0, random_roll=True, warp=True,
    ),
    channel_map=ECG_REST_LEADS,
    cacheable=False,
)
TMAPS["ecg_rest_raw_100"] = TensorMap(
    ...
    shape=(5000, 12),
    path_prefix="ukb_ecg_rest",
    tensor_from_file=_make_ecg_rest(population_normalize=100.0),
    channel_map=ECG_REST_LEADS,
)
...
def _make_rhythm_tensor(skip_poor=True):
    def rhythm_tensor_from_file(tm, hd5, dependents={}):
        ...
        ecg_interpretation = str(
            tm.hd5_first_dataset_in_group(hd5, "ukb_ecg_rest/ecg_rest_text/")[()],
        )
...
TMAPS["ecg_rhythm"] = TensorMap(
    "ecg_rhythm",
    Interpretation.CATEGORICAL,
    tensor_from_file=_make_rhythm_tensor(),
...
TMAPS["ecg_rhythm_poor"] = TensorMap(
    "ecg_rhythm",
    Interpretation.CATEGORICAL,
    tensor_from_file=_make_rhythm_tensor(False),
...
TMAPS["ecg_rest_age"] = TensorMap(
    "ecg_rest_age",
    Interpretation.CONTINUOUS,
    tensor_from_file=_age_in_years_tensor("ecg_rest_date"),
    loss="logcosh",
    channel_map={"ecg_rest_age": 0},
    validator=make_range_validator(0, 110),
    normalization={"mean": 65, "std": 7.7},
)
...
TMAPS["acute_mi"] = TensorMap(
    ...
    channel_map={"no_acute_mi": 0, "ACUTE MI": 1},
...
TMAPS["av_block"] = TensorMap(
    ...
    Interpretation.CATEGORICAL,
    tensor_from_file=label_from_ecg_interpretation_text,
    channel_map={"no_av_block": 0, "st degree AV block": 1},
...
TMAPS["lvh_fine"] = TensorMap(
    ...
    Interpretation.CATEGORICAL,
    tensor_from_file=label_from_ecg_interpretation_text,
    loss=weighted_crossentropy([0.5, 12.0, 16.0, 30.0, 36.0], "lvh_fine"),
    channel_map={
        "no_lvh_fine": 0,
        "Minimal voltage criteria for LVH may be normal variant": 1,
        "Moderate voltage criteria for LVH may be normal variant": 2,
        "Voltage criteria for left ventricular hypertrophy": 3,
...
TMAPS["poor_data_quality"] = TensorMap(
    ...
    Interpretation.CATEGORICAL,
    tensor_from_file=label_from_ecg_interpretation_text,
    channel_map={"no_poor_data_quality": 0, "Poor data quality": 1},
...
TMAPS["prolonged_qt"] = TensorMap(
    ...
    Interpretation.CATEGORICAL,
    tensor_from_file=label_from_ecg_interpretation_text,
    channel_map={"no_prolonged_qt": 0, "Prolonged QT": 1},
...
TMAPS["ecg_rest_ramplitude_raw"] = TensorMap(
    ...
    path_prefix="ukb_ecg_rest",
    shape=(12,),
    tensor_from_file=_make_ukb_ecg_rest(1.0),
    loss="logcosh",
...
TMAPS["ecg_rest_ramplitude"] = TensorMap(
    ...
    path_prefix="ukb_ecg_rest",
    shape=(12,),
    tensor_from_file=_make_ukb_ecg_rest(),
    loss="logcosh",
    metrics=["mse", "mape", "mae"],
    loss_weight=1.0,
)
...
def _make_ukb_ecg_rest_lvh():
    def ukb_ecg_rest_lvh_from_file(tm, hd5, dependents={}):
        ...
        if "ukb_ecg_rest" not in hd5:
            raise ValueError("Group with R and S amplitudes not present in hd5")
        tensor_ramp = _get_tensor_at_first_date(
            hd5, tm.path_prefix, "ramplitude", _pass_nan,
        )
        tensor_samp = _get_tensor_at_first_date(
            hd5, tm.path_prefix, "samplitude", _pass_nan,
        )
        criteria_sleads = [lead_order[l] for l in ["V1", "V3"]]
        criteria_rleads = [lead_order[l] for l in ["aVL", "V5", "V6"]]
        if np.any(
            np.isnan(
                np.union1d(tensor_ramp[criteria_rleads], tensor_samp[criteria_sleads]),
            ),
        ):
            raise ValueError(
        ...
        if tm.name == "avl_lvh":
            ...
            is_lvh = (
                tensor_samp[lead_order["V1"]]
                + np.maximum(
                    tensor_ramp[lead_order["V5"]], tensor_ramp[lead_order["V6"]],
                )
                > sl_min
            )
...
TMAPS["ecg_rest_lvh_avl"] = TensorMap(
    ...
    Interpretation.CATEGORICAL,
    path_prefix="ukb_ecg_rest",
    tensor_from_file=_make_ukb_ecg_rest_lvh(),
...
def _ecg_rest_to_segment(population_normalize=None, hertz=500, random_offset_seconds=0):
    def ecg_rest_section_to_segment(tm, hd5, dependents={}):
        ...
        segment_index = np.array(
            segmented[
                random_offset_samples : random_offset_samples
                + tm.dependent_map.shape[0]
            ],
            dtype=np.float32,
        )
        dependents[tm.dependent_map] = to_categorical(
        ...
        for k in hd5[tm.path_prefix]:
            if k in tm.channel_map:
                tensor[:, tm.channel_map[k]] = np.array(
                    hd5[tm.path_prefix][k], dtype=np.float32,
...
TMAPS["ecg_segmented"] = TensorMap(
    "ecg_segmented",
    Interpretation.CATEGORICAL,
    shape=(1224, len(ECG_SEGMENTED_CHANNEL_MAP)),
    path_prefix="ecg_rest",
    cacheable=False,
    channel_map=ECG_SEGMENTED_CHANNEL_MAP,
)
TMAPS["ecg_section_to_segment"] = TensorMap(
    "ecg_section_to_segment",
    shape=(1224, 12),
    path_prefix="ecg_rest",
    dependent_map=TMAPS["ecg_segmented"],
    channel_map=ECG_REST_LEADS,
    tensor_from_file=_ecg_rest_to_segment(),
)
...
TMAPS["ecg_segmented_second"] = TensorMap(
    "ecg_segmented",
    Interpretation.CATEGORICAL,
    shape=(496, len(ECG_SEGMENTED_CHANNEL_MAP)),
    path_prefix="ecg_rest",
    cacheable=False,
    channel_map=ECG_SEGMENTED_CHANNEL_MAP,
)
TMAPS["ecg_second_to_segment"] = TensorMap(
    "ecg_second_to_segment",
    shape=(496, 12),
    path_prefix="ecg_rest",
    dependent_map=TMAPS["ecg_segmented_second"],
    cacheable=False,
    channel_map=ECG_REST_LEADS,
    tensor_from_file=_ecg_rest_to_segment(random_offset_seconds=1.5),
)
...
TMAPS["t2_flair_slice_1"] = TensorMap(
    ...
    shape=(256, 256, 1),
    path_prefix="ukb_brain_mri",
    tensor_from_file=_random_slice_tensor("t2_flair_sag_p2_1mm_fs_ellip_pf78_1"),
    normalization={"zero_mean_std1": True},
)
TMAPS["t2_flair_slice_2"] = TensorMap(
    ...
    shape=(256, 256, 1),
    path_prefix="ukb_brain_mri",
    tensor_from_file=_random_slice_tensor("t2_flair_sag_p2_1mm_fs_ellip_pf78_2"),
    normalization={"zero_mean_std1": True},
)
...
TMAPS["swi_total_mag_te2_orig"] = TensorMap(
...
TMAPS["swi_total_mag_orig"] = TensorMap(
    "SWI_TOTAL_MAG_orig",
    shape=(256, 288, 48),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
...
TMAPS["brain_mask_normed"] = TensorMap(
    "brain_mask_normed",
    shape=(256, 288, 48),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
...
TMAPS["filtered_phase"] = TensorMap(
    "filtered_phase",
    shape=(256, 288, 48),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
...
TMAPS["t1_brain_30_slices"] = TensorMap(
    "t1_brain_30_slices",
    shape=(192, 256, 30),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T1_brain", 66, 126, 2, pad_shape=(192, 256, 256),
    ),
)
TMAPS["t1_30_slices"] = TensorMap(
    "t1_30_slices",
    shape=(192, 256, 30),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor("T1", 90, 150, 2, pad_shape=(192, 256, 256)),
)
TMAPS["t1_30_slices_4d"] = TensorMap(
    "t1_30_slices_4d",
    shape=(192, 256, 30, 1),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T1", 90, 150, 2, pad_shape=(192, 256, 256, 1),
    ),
)
TMAPS["t1_30_slices_fs"] = TensorMap(
    ...
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T1", 90, 150, 2, pad_shape=(192, 256, 256), flip_swap=True, swap_axes=1,
    ),
)
TMAPS["t1_30_slices_4d_fs"] = TensorMap(
    ...
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T1", 90, 150, 2, pad_shape=(192, 256, 256, 1), flip_swap=True, swap_axes=1,
    ),
)

TMAPS["t1_brain_to_mni"] = TensorMap(
    "T1_brain_to_MNI",
    shape=(192, 256, 256, 1),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
...
TMAPS["t1_fast_t1_brain_bias"] = TensorMap(
...
TMAPS["t2_flair_brain"] = TensorMap(
    "T2_FLAIR_brain",
    shape=(192, 256, 256, 1),
    path_prefix="ukb_brain_mri",
    normalization={"zero_mean_std1": True},
...
TMAPS["t2_flair_brain_30_slices"] = TensorMap(
    ...
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T2_FLAIR_brain", 66, 126, 2, pad_shape=(192, 256, 256),
    ),
)
TMAPS["t2_flair_30_slices"] = TensorMap(
    ...
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T2_FLAIR", 90, 150, 2, pad_shape=(192, 256, 256),
    ),
)
TMAPS["t2_flair_30_slices_4d"] = TensorMap(
    ...
    path_prefix="ukb_brain_mri",
    tensor_from_file=_slice_subset_tensor(
        "T2_FLAIR", 90, 150, 2, pad_shape=(192, 256, 256, 1),
    ),
    normalization={"zero_mean_std1": True},
)
TMAPS["t2_flair_30_slices_fs"] = TensorMap(
    ...
    normalization={"zero_mean_std1": True},
    tensor_from_file=_slice_subset_tensor(
        "T2_FLAIR", 90, 150, 2, pad_shape=(192, 256, 256), flip_swap=True, swap_axes=1,
    ),
)
...
TMAPS["t2_flair_unbiased_brain"] = TensorMap(
...
TMAPS["t1_seg_30_slices"] = TensorMap(
    ...
    tensor_from_file=_mask_subset_tensor(
        "T1_fast_T1_brain_seg", 90, 150, 2, pad_shape=(192, 256, 256, 1),
    ),
    channel_map={"not_brain_tissue": 0, "csf": 1, "grey": 2, "white": 3},
)
TMAPS["t1_brain_mask_30_slices"] = TensorMap(
    ...
    path_prefix="ukb_brain_mri",
    tensor_from_file=_mask_subset_tensor(
        "T1_brain_mask", 90, 150, 2, pad_shape=(192, 256, 256, 1),
    ),
    channel_map={"not_brain": 0, "brain": 1},
)
TMAPS["lesions"] = TensorMap(
    ...
    path_prefix="ukb_brain_mri",
    tensor_from_file=_mask_from_file,
    channel_map={"not_lesion": 0, "lesion": 1},
    loss=weighted_crossentropy([0.01, 10.0], "lesion"),
)


def _combined_subset_tensor(
    tensor_keys, start, stop, step=1, pad_shape=None, flip_swap=False,
):
    slice_subsets = [
...
TMAPS["t1_and_t2_flair_30_slices"] = TensorMap(
    ...
    tensor_from_file=_combined_subset_tensor(
        ["T1", "T2_FLAIR"], 90, 150, 2, pad_shape=(192, 256, 256),
    ),
    normalization={"zero_mean_std1": True},
)
...
TMAPS["t1_t2_dicom_30_slices"] = TensorMap(
    ...
    path_prefix="ukb_brain_mri",
    tensor_from_file=_combined_subset_tensor(
        _dicom_keys, 130, 190, 2, pad_shape=(192, 256, 256), flip_swap=True,
    ),
    normalization={"zero_mean_std1": True},
)
TMAPS["t1_t2_dicom_50_slices"] = TensorMap(
    ...
    path_prefix="ukb_brain_mri",
    tensor_from_file=_combined_subset_tensor(
        _dicom_keys, 100, 200, 2, pad_shape=(192, 256, 256), flip_swap=True,
    ),
    normalization={"zero_mean_std1": True},
)
...
TMAPS["ttntv"] = TensorMap(
    "has_ttntv",
    Interpretation.CATEGORICAL,
    channel_map={"no_TTN_tv": 0, "TTN_tv": 1},
    tensor_from_file=_ttn_tensor_from_file,
)
TMAPS["ttntv_10x"] = TensorMap(
    "has_ttntv",
    Interpretation.CATEGORICAL,
    channel_map={"no_TTN_tv": 0, "TTN_tv": 1},
    loss_weight=10.0,
...
TMAPS["myocardial_mass_noheritable_men_only"] = TensorMap(
    ...
    normalization={"mean": 100.0, "std": 18.0},
)
TMAPS["myocardial_mass_noheritable_women_only"] = TensorMap(
    ...
    channel_map={"inferred_myocardial_mass_noheritable": 0},
    normalization={"mean": 78.0, "std": 16.0},
)


def _make_lvh_from_lvm_tensor_from_file(
    lvm_key, group_key="continuous", male_lvh_threshold=72, female_lvh_threshold=55,
):
...
TMAPS["adjusted_myocardium_mass_indexed"] = TensorMap(
    ...
    validator=make_range_validator(0, 400),
    loss="logcosh",
    channel_map={"adjusted_myocardium_mass_indexed": 0},
    path_prefix="continuous",
    normalization={"mean": 89.70, "std": 24.80},
)
TMAPS["lvh_from_indexed_lvm_parented"] = TensorMap(
    ...
    parents=[
        TMAPS["adjusted_myocardium_mass_indexed"],
        TMAPS["adjusted_myocardium_mass"],
    ],
)
...
def _mri_hd5_to_structured_grids(hd5, name, save_path=None, order="F"):
    ...
    _, dataset_indices, dataset_counts = np.unique(
        orientations, axis=1, return_index=True, return_counts=True,
    )
    grids = []
    for d_idx, d_cnt in zip(dataset_indices, dataset_counts):
        ...
        grids[-1].SetExtent(
            0, len(x_coors) - 1, 0, len(y_coors) - 1, 0, len(z_coors) - 1,
        )
        for t in range(MRI_FRAMES):
            arr_vtk = vtk.util.numpy_support.numpy_to_vtk(
                arr[:, :, d_idx : d_idx + d_cnt, t].ravel(order=order), deep=True,
            )
...
def _map_points_to_cells(pts, dataset, tol=1e-3):
    ...
    for pt_id, pt in enumerate(pts):
        if locator.FindClosestPointWithinRadius(
            pt, tol, closest_pt, generic_cell, cell_id, sub_id, dist2, inside,
        ):
...
def _make_mri_projected_segmentation_from_file(
    to_segment_name, segmented_name, save_path=None,
):
            ...
            for ds_j, ds_to_segment in enumerate(cine_to_segment_grids):
                ...
                for s in range(dims[2] - 1):
                    slice_center = np.mean(
                        pts[s * npts_per_slice : (s + 2) * npts_per_slice], axis=0,
                    )
                    slice_cell_pts = cell_pts[
                        s * ncells_per_slice : (s + 1) * ncells_per_slice
                    ]
                    ...
                    for t in range(MRI_FRAMES):
                        ...
                        if len(tm.shape) == 3:
                            tensor[:, :, t] = np.maximum(
                                tensor[:, :, t],
                                projected_arr.reshape(tm.shape[0], tm.shape[1]),
                            )
                        elif len(tm.shape) == 4:
                    ...
                    if save_path:
                        ...
                        writer_segmented.SetFileName(
                            os.path.join(
                                save_path, f"{tm.name}_segmented_{ds_i}_{ds_j}_{s}.vtp",
                            ),
                        )
...
def _slice_tensor(tensor_key, slice_index):
    def _slice_tensor_from_file(tm, hd5, dependents={}):
        if tm.shape[-1] == 1:
            t = _pad_or_crop_array_to_shape(
                tm.shape[:-1],
                np.array(hd5[tensor_key][..., slice_index], dtype=np.float32),
            )
            tensor = np.expand_dims(t, axis=-1)
        else:
            tensor = _pad_or_crop_array_to_shape(
                tm.shape, np.array(hd5[tensor_key][..., slice_index], dtype=np.float32),
            )
        return tensor
...
def _pad_crop_tensor(tm, hd5, dependents={}):
    return _pad_or_crop_array_to_shape(
        tm.shape,
        np.array(
...
def _segmented_dicom_slices(dicom_key_prefix, path_prefix="ukb_cardiac_mri"):
    def _segmented_dicom_tensor_from_file(tm, hd5, dependents={}):
        ...
        if path_prefix == "ukb_liver_mri":
            categorical_index_slice = _get_tensor_at_first_date(
                hd5, path_prefix, f"{dicom_key_prefix}1",
            )
            categorical_one_hot = to_categorical(
                categorical_index_slice, len(tm.channel_map),
            )
            tensor[..., :] = _pad_or_crop_array_to_shape(
                tensor[..., :].shape, categorical_one_hot,
            )
        elif tm.axes() == 4:
            for i in range(tm.shape[-2]):
                categorical_index_slice = _get_tensor_at_first_date(
                    hd5, path_prefix, f"{dicom_key_prefix}{i+1}",
                )
                categorical_one_hot = to_categorical(
                    categorical_index_slice, len(tm.channel_map),
                )
                tensor[..., i, :] = _pad_or_crop_array_to_shape(
                    tensor[..., i, :].shape, categorical_one_hot,
                )
        else:
...
TMAPS["lax_4ch_segmented_192_w"] = TensorMap(
    ...
    loss=weighted_crossentropy(
        [0.01, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 1.0, 1.0, 1.0, 1.0, 5.0, 0.5],
    ),
)
...
TMAPS["cine_segmented_ao_dist"] = TensorMap(
    "cine_segmented_ao_dist",
    Interpretation.CATEGORICAL,
    shape=(160, 192, 100, len(MRI_AO_SEGMENTED_CHANNEL_MAP)),
    tensor_from_file=_segmented_dicom_slices("cine_segmented_ao_dist_annotated_"),
    channel_map=MRI_AO_SEGMENTED_CHANNEL_MAP,
)
...
def _make_fallback_tensor_from_file(tensor_keys):
    def fallback_tensor_from_file(tm, hd5, dependents={}):
        for k in tensor_keys:
            if k in hd5:
                return _pad_or_crop_array_to_shape(
                    tm.shape, np.array(hd5[k], dtype=np.float32),
                )
...
TMAPS["weight_lbs_2"] = TensorMap(
    "weight_lbs",
    Interpretation.CONTINUOUS,
    normalization={"mean": 168.74, "std": 34.1},
    loss="logcosh",
    channel_map={"weight_lbs": 0},
    tensor_from_file=preprocess_with_function(
        lambda x: x * 2.20462, "continuous/21002_Weight_2_0",
    ),
)
...
TMAPS["weekly_alcohol_0"] = TensorMap(
    "weekly_alcohol_0",
    loss="logcosh",
    path_prefix="continuous",
...
TMAPS["weekly_alcohol_1"] = TensorMap(
    "weekly_alcohol_1",
    loss="logcosh",
    path_prefix="continuous",
...
TMAPS["weekly_alcohol_2"] = TensorMap(
    "weekly_alcohol_2",
    loss="logcosh",
    path_prefix="continuous",
    ...
    def sax_tensor_from_file(tm, hd5, dependents={}):
        ...
        for b in range(tm.shape[-2]):
            try:
                ...
                tensor[:, :, b, 0] = _pad_or_crop_array_to_shape(
                    tm_shape,
                    np.array(hd5[f"{b_series_prefix}_frame_b{b}"], dtype=np.float32),
                )
                index_tensor = _pad_or_crop_array_to_shape(
                    tm_shape,
                    np.array(hd5[f"{b_series_prefix}_mask_b{b}"], dtype=np.float32),
                )
                dependents[tm.dependent_map][:, :, b, :] = to_categorical(
                    index_tensor, tm.dependent_map.shape[-1],
                )
            except KeyError:
                missing += 1
                tensor[:, :, b, 0] = 0
                dependents[tm.dependent_map][
                    :, :, b, MRI_SEGMENTED_CHANNEL_MAP["background"]
                ] = 1
        if missing == tm.shape[-2]:
            raise ValueError(
                f"Could not find any slices in {tm.name} was hoping for {tm.shape[-2]}",
            )
        return tensor
...
TMAPS["sax_all_diastole"] = TensorMap(
    "sax_all_diastole",
    shape=(256, 256, 13, 1),
    tensor_from_file=sax_tensor("diastole"),
...
TMAPS["sax_all_systole_segmented"] = TensorMap(
    "sax_all_systole_segmented",
    Interpretation.CATEGORICAL,
    shape=(256, 256, 13, 3),
    channel_map=MRI_SEGMENTED_CHANNEL_MAP,
)
TMAPS["sax_all_systole_segmented_weighted"] = TensorMap(
    "sax_all_systole_segmented_weighted",
    Interpretation.CATEGORICAL,
    shape=(256, 256, 13, 3),
    channel_map=MRI_SEGMENTED_CHANNEL_MAP,
    ...
    def sax_tensor_from_file(tm, hd5, dependents={}):
        ...
        for b in range(total_b_slices):
            try:
                tm_shape = (tm.shape[0], tm.shape[1])
                tensor[:, :, b, 0] = _pad_or_crop_array_to_shape(
                    tm_shape, np.array(hd5[f"diastole_frame_b{b}"], dtype=np.float32),
                )
                index_tensor = _pad_or_crop_array_to_shape(
                    tm_shape, np.array(hd5[f"diastole_mask_b{b}"], dtype=np.float32),
                )
                dependents[tm.dependent_map][:, :, b, :] = to_categorical(
                    index_tensor, tm.dependent_map.shape[-1],
                )
                tensor[:, :, b + total_b_slices, 0] = _pad_or_crop_array_to_shape(
                    tm_shape, np.array(hd5[f"systole_frame_b{b}"], dtype=np.float32),
                )
                index_tensor = _pad_or_crop_array_to_shape(
                    tm_shape, np.array(hd5[f"systole_mask_b{b}"], dtype=np.float32),
                )
                dependents[tm.dependent_map][
                    :, :, b + total_b_slices, :
                ] = to_categorical(index_tensor, tm.dependent_map.shape[-1])
            except KeyError:
                missing += 1
                tensor[:, :, b, 0] = 0
                dependents[tm.dependent_map][
...
TMAPS["sax_all_segmented"] = TensorMap(
    "sax_all_segmented",
    Interpretation.CATEGORICAL,
    shape=(256, 256, 26, 3),
    channel_map=MRI_SEGMENTED_CHANNEL_MAP,
...
TMAPS["sax_all_segmented_weighted"] = TensorMap(
    "sax_all_segmented_weighted",
    Interpretation.CATEGORICAL,
    shape=(256, 256, 26, 3),
    channel_map=MRI_SEGMENTED_CHANNEL_MAP,
    loss=weighted_crossentropy([1.0, 40.0, 40.0], "sax_all_segmented"),
)

TMAPS["sax_all"] = TensorMap(
    "sax_all",
    shape=(256, 256, 26, 1),
    tensor_from_file=all_sax_tensor(),
    dependent_map=TMAPS["sax_all_segmented"],
)
TMAPS["sax_all_weighted"] = TensorMap(
    "sax_all_weighted",
    shape=(256, 256, 26, 1),
    tensor_from_file=all_sax_tensor(),
    dependent_map=TMAPS["sax_all_segmented_weighted"],
)


def _segmented_index_slices(
    key_prefix: str, shape: Tuple[int], path_prefix: str = "ukb_cardiac_mri",
) -> Callable:
...
def _bounding_box_from_categorical(
    segmented_shape: Tuple[int], segmented_key: str, class_index: int,
) -> Callable:
...
def _bounding_box_from_callable(
    class_index: int, tensor_from_file_fxn: Callable,
) -> Callable:
...
TMAPS["lax_3ch_lv_cavity_bbox_slice0"] = TensorMap(
    "lax_3ch_lv_cavity_bbox_slice0",
    Interpretation.MESH,
    shape=(4,),
    tensor_from_file=_bounding_box_from_categorical(
        (160, 160),
        "ukb_cardiac_mri/cine_segmented_lax_3ch_annotated_1/instance_0",
...
TMAPS["lax_3ch_left_atrium_bbox_slice0"] = TensorMap(
    "lax_3ch_left_atrium_bbox_slice0",
    Interpretation.MESH,
    shape=(4,),
    tensor_from_file=_bounding_box_from_categorical(
        (160, 160),
...
aorta_descending_tff = _bounding_box_from_categorical(
    (192, 224),
    "ukb_cardiac_mri/cine_segmented_ao_dist_annotated_1/instance_0",
    MRI_AO_SEGMENTED_CHANNEL_MAP["descending_aorta"],
)
TMAPS["cine_segmented_ao_descending_aorta_bbox_slice0"] = TensorMap(
    "cine_segmented_ao_descending_aorta_bbox_slice0",
    Interpretation.MESH,
    shape=(4,),
...
aorta_ascending_tff = _bounding_box_from_categorical(
    (192, 224),
    "ukb_cardiac_mri/cine_segmented_ao_dist_annotated_1/instance_0",
    MRI_AO_SEGMENTED_CHANNEL_MAP["ascending_aorta"],
)
TMAPS["cine_segmented_ao_ascending_aorta_bbox_slice0"] = TensorMap(
    "cine_segmented_ao_ascending_aorta_bbox_slice0",
    Interpretation.MESH,
    shape=(4,),
...
TMAPS["lax_3ch_lv_cavity_bbox"] = TensorMap(
    "lax_3ch_lv_cavity_bbox",
    Interpretation.MESH,
    shape=(6,),
    channel_map=_bounding_box_channel_map(3),
    tensor_from_file=_bounding_box_from_callable(
        5, _segmented_index_slices("cine_segmented_lax_3ch_annotated_", (192, 160, 50)),
    ),
)

bbfc = _bounding_box_from_callable(
    MRI_AO_SEGMENTED_CHANNEL_MAP["descending_aorta"],
    _segmented_index_slices("cine_segmented_ao_dist_annotated_", (192, 224, 100)),
)
TMAPS["cine_segmented_ao_descending_aorta_bbox"] = TensorMap(
    "cine_segmented_ao_descending_aorta_bbox",
    Interpretation.MESH,
    shape=(6,),
    tensor_from_file=bbfc,
    channel_map=_bounding_box_channel_map(3),
)
...
TMAPS["cine_segmented_ao_ascending_aorta_bbox"] = TensorMap(
    "cine_segmented_ao_ascending_aorta_bbox",
    Interpretation.MESH,
    shape=(6,),
    tensor_from_file=abbfc,
    channel_map=_bounding_box_channel_map(3),
)
