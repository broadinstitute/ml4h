...
def build_partners_tensor_maps(needed_tensor_maps: List[str]) -> Dict[str, TensorMap]:
    ...
    for diagnosis in diagnosis2column:
        ...
        if name in needed_tensor_maps:
            tensor_from_file_fxn = tff = _survival_from_file(days_window, INCIDENCE_CSV, diagnosis_column=diagnosis2column[diagnosis])
        ...
        if name in needed_tensor_maps:
            tensor_from_file_fxn = _survival_from_file(days_window, INCIDENCE_CSV, diagnosis_column=diagnosis2column[diagnosis], incidence_only=True)
