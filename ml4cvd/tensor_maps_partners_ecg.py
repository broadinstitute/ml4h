            ...
            for cm in tm.channel_map:
                try:
                    ...
                    voltage = _resample_voltage(
                        voltage, shape[1] if dynamic else shape[0]
                    )
                    slices = (
                        (i, ..., tm.channel_map[cm])
                        if dynamic
...
TMAPS["partners_ecg_2500_newest"] = TensorMap(
    "ecg_rest_2500_newest",
    shape=(2500, 12),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=make_voltage(),
    normalization={"zero_mean_std1": True},
...
TMAPS["partners_ecg_5000_newest"] = TensorMap(
    "ecg_rest_5000_newest",
    shape=(5000, 12),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=make_voltage(),
    normalization={"zero_mean_std1": True},
...
TMAPS["partners_ecg_2500_raw_newest"] = TensorMap(
    "ecg_rest_2500_raw_newest",
    shape=(2500, 12),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=make_voltage(population_normalize=2000.0),
    channel_map=ECG_REST_AMP_LEADS,
)
TMAPS["partners_ecg_5000_raw_newest"] = TensorMap(
    "ecg_rest_5000_raw_newest",
    shape=(5000, 12),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=make_voltage(population_normalize=2000.0),
    channel_map=ECG_REST_AMP_LEADS,
        ...
        for i, ecg_date in enumerate(ecg_dates):
            ...
            try:
                tensor[i] = decompress_data(
                    data_compressed=hd5[path][()], dtype="str"
                )
            except KeyError:
                logging.debug(
...
def partners_ecg_age(tm, hd5, dependents={}):
    ...
    for i, ecg_date in enumerate(ecg_dates):
        ...
        try:
            ...
            delta = _partners_str2date(acquisition) - _partners_str2date(
                birthday
            )
...
TMAPS["partners_ecg_age_newest"] = TensorMap(
...
TMAPS["partners_ecg_acquisition_year_newest"] = TensorMap(
    "partners_ecg_acquisition_year_newest",
    path_prefix=PARTNERS_PREFIX,
    loss="logcosh",
    tensor_from_file=partners_ecg_acquisition_year,
    shape=(1,),
...
def partners_bmi(tm, hd5, dependents={}):
    ...
    for i, ecg_date in enumerate(ecg_dates):
        ...
        try:
            ...
            logging.info(
                f" Height was {height_in} weight: {weight_lbs} bmi is {bmi}"
            )
            tensor[i] = bmi
        except KeyError:
            pass
...
TMAPS["partners_ecg_bmi_newest"] = TensorMap(
    "partners_ecg_bmi_newest",
    path_prefix=PARTNERS_PREFIX,
    channel_map={"bmi": 0},
    tensor_from_file=partners_bmi,
)
        ...
        for i, ecg_date in enumerate(ecg_dates):
            ...
            try:
                ...
                for key in tm.channel_map:
                    slices = (
                        (i, tm.channel_map[key])
                        if dynamic
                        else (tm.channel_map[key],)
                    )
...
TMAPS["lead_i_zeros_newest"] = TensorMap(
    "lead_i_zeros_newest",
    shape=(1,),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=voltage_zeros,
...
TMAPS["lead_v6_zeros_newest"] = TensorMap(
    "lead_v6_zeros_newest",
    shape=(1,),
    path_prefix=PARTNERS_PREFIX,
    tensor_from_file=voltage_zeros,
    channel_map={"V6": 0},
        ...
        with open(file_name, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    patient_table[patient_key] = _loyalty_str2date(
                        row[start_index]
                    )
                    birth_table[patient_key] = _loyalty_str2date(
                        row[birth_index]
                    )
                    if row[date_index] == "" or row[date_index] == "NULL":
                        continue
                    date_table[patient_key] = _loyalty_str2date(
                        row[date_index]
                    )
                    if len(patient_table) % 2000 == 0:
                        logging.debug(
                            f"Processed: {len(patient_table)} patient rows."
                        )
...
def loyalty_time_to_event(
    ...
    try:
        with open(file_name, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    patient_key = int(row[patient_index])
                    disease_dicts["follow_up_start"][
                        patient_key
                    ] = _loyalty_str2date(row[follow_up_start_index])
                    disease_dicts["follow_up_total"][patient_key] = float(
                        row[follow_up_total_index]
                    )
                    if row[date_index] == "" or row[date_index] == "NULL":
                        continue
                    disease_dicts["diagnosis_dates"][
                        patient_key
                    ] = _loyalty_str2date(row[date_index])
    ...
    def _cox_tensor_from_file(tm: TensorMap, hd5: h5py.File, dependents=None):
        ...
        for i, ecg_date in enumerate(ecg_dates):
            ...
            if (
                assess_date
                < disease_dicts["follow_up_start"][patient_key_from_ecg]
            ):
            ...
            if incidence_only and censor_date <= assess_date and has_disease:
                raise ValueError(
                    f"{tm.name} only considers incident diagnoses"
                )
            ...
            for i, day_delta in enumerate(
                ...
                if (
                    ...
                    if has_disease and incidence_only:
                        raise ValueError(
                            f"{tm.name} is skipping prevalent cases."
                        )
            logging.debug(
...
def build_partners_tensor_maps(
    ...
    for diagnosis in diagnosis2column:
        ...
        if name in needed_tensor_maps:
            ...
            name2tensormap[name] = TensorMap(
                ...
                Interpretation.CATEGORICAL,
                path_prefix=PARTNERS_PREFIX,
                channel_map=_diagnosis_channels(
                    diagnosis, incidence_only=True
                ),
                tensor_from_file=tensor_from_file_fxn,
            )
...
def _cardiac_surgery_str2date(input_date: str, date_format: str="%d%b%Y") -> datetime.datetime:
    return datetime.datetime.strptime(input_date, date_format)


def _dates_with_voltage_len(
    ecg_dates,
    voltage_len,
    tm,
    hd5,
    voltage_key=list(ECG_REST_AMP_LEADS.keys())[0],
):
    path = lambda ecg_date: _make_hd5_path(tm, ecg_date, voltage_key)
    return [
        ecg_date
        for ecg_date in ecg_dates
        if hd5[path(ecg_date)].attrs["len"] == voltage_len
    ]
        ...
        with open(file_name, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    surgery_date_table[
                        patient_key
                    ] = _cardiac_surgery_str2date(row[date_index])
                    for outcome in outcome2column:
                        outcome_table[outcome][patient_key] = int(
                            row[header.index(outcome2column[outcome])]
                        )
                    if len(outcome_table) % 1000 == 0:
                        logging.debug(
                            f"Processed: {len(outcome_table)} outcome rows."
                        )
                except ValueError as e:
                    logging.debug(f"Value error {e}")

        logging.info(
            f"Processed outcomes:{list(outcome_table.keys())}. Got {len(surgery_date_table)} patients."
        )
    ...
    def tensor_from_file(tm: TensorMap, hd5: h5py.File, dependents=None):
        ...
        if require_exact_length:
            ecg_dates = _dates_with_voltage_len(
                ecg_dates, tm.shape[0], tm, hd5
            )
        tensor = np.zeros(tm.shape, dtype=np.float32)
        for dtm in tm.dependent_map:
            dependents[tm.dependent_map[dtm]] = np.zeros(
                tm.dependent_map[dtm].shape, dtype=np.float32
            )
        ecg_date = _date_in_window_from_dates(
            ecg_dates, surgery_date_table[mrn_int], day_window, most_recent_ecg
        )
        for cm in tm.channel_map:
            path = _make_hd5_path(tm, ecg_date, cm)
            voltage = decompress_data(
            ...
            if require_exact_length and len(voltage) != tm.shape[0]:
                raise ValueError(
                    f"lead {cm} voltage length {len(voltage)} did not match required length {tm.shape[0]}"
                )
            voltage = _resample_voltage(voltage, tm.shape[0])
            tensor[..., tm.channel_map[cm]] = voltage

        for dtm in tm.dependent_map:
            dependents[tm.dependent_map[dtm]][
                outcome_table[dtm][mrn_int]
            ] = 1.0
...
def build_cardiac_surgery_tensor_maps(
    ...
    for outcome in outcome2column:
        ...
        dependent_maps[outcome] = TensorMap(
            outcome,
            Interpretation.CATEGORICAL,
            path_prefix=PARTNERS_PREFIX,
    ...
    name = "ecg_2500_sts_exact_random_from_window"
    if name in needed_tensor_maps:
        name2tensormap[name] = TensorMap(
            name,
            shape=(2500, 12),
    ...
    name = "ecg_5000_sts_exact_random_from_window"
    if name in needed_tensor_maps:
        name2tensormap[name] = TensorMap(
            name,
            shape=(5000, 12),
    ...
    with open(file_name, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            try:
                patient_key = int(row[patient_index])
                start_date = (
                    _cardiac_surgery_str2date(row[start_index], "%m/%d/%y")
                    + datetime.timedelta(days=start_offset)
                ).strftime(PARTNERS_DATETIME_FORMAT)
                end_date = (
                    _cardiac_surgery_str2date(row[end_index], "%m/%d/%y")
...
def _build_cardiac_surgery_basic_tensor_maps(
    ...
    date_interval_lookup = build_date_interval_lookup(
        file_name="/data/sts-afib/mgh-afib-after-avr-metadata.csv",
        patient_column="mrn",
        start_column="surgery_date",
        start_offset=0,
        end_column="surgery_date",
        end_offset=180,
    )
