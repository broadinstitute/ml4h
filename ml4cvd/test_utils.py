...
MULTIMODAL_UP_TO_4D = [
    list(x) for x in product(CONTINUOUS_TMAPS[:-1], CATEGORICAL_TMAPS[:-1])
]
SEGMENT_IN = TensorMap(
    f"2d_for_segment_in",
    shape=(32, 32, 1),
    interpretation=Interpretation.CONTINUOUS,
    metrics=["mse"],
)
SEGMENT_OUT = TensorMap(
...
TMAPS = {tmap.name: tmap for tmap in CONTINUOUS_TMAPS + CATEGORICAL_TMAPS}
PARENT_TMAPS = [
...
CYCLE_PARENTS = [
    TensorMap(
        f"parent_test_cycle_{i}", shape=(1,), interpretation=Interpretation.CONTINUOUS,
    )
    for i in range(3)
]
