...
def plot_survival(prediction, truth, title, days_window=1825, prefix='./figures/', paths=None):
    ...
    predicted_proportion = np.sum((truth[:, :intervals]+truth[:, intervals:])*np.cumprod(prediction[:, :intervals], axis=1), axis=0) / (alive_per_step + sick_per_step)
    ...
    plt.plot(range(0, days_window, 1 + days_window // intervals), survivorship, marker='o', label='True Proportion')
