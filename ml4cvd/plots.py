...
def evaluate_predictions(tm: TensorMap, y_predictions: np.ndarray, y_truth: np.ndarray, title: str, folder: str, test_paths: List[str] = None,
    ...
    if tm.is_categorical() and tm.axes() == 1:
        ...
        performance_metrics.update(plot_roc_per_class(y_predictions, y_truth, {'event': 0}, f'{title}_C_Index_{c_index:0.3f}', folder))
...
def plot_survival_curves(prediction, truth, title, days_window=1825, prefix='./figures/', num_curves=45, paths=None):
    intervals = truth.shape[-1] // 2
