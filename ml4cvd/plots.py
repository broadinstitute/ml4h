...
def plot_survival(prediction, truth, title, days_window=1825, prefix='./figures/', paths=None):
    ...
    survivorship = np.cumprod(1 - (sick_per_step / alive_per_step), axis=1)
    ...
    logging.info(f"Survivors at each step is: {np.sum(truth[:, :intervals], axis=0)} out of {truth.shape[0]}")
    ...
    true_proportion = cumulative_sick / truth.shape[0]  # (cumulative_sick + np.sum(truth[:, :intervals], axis=0))
