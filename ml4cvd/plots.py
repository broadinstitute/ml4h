...
def plot_survival(prediction, truth, title, days_window=1825, prefix='./figures/', paths=None):
    ...
    logging.info(f"Cumulative Censored: {cumulative_censored}")
    logging.info(f"Survivorship: {survivorship}")
    predicted_proportion = np.cumsum(np.sum(np.cumprod((truth[:, :intervals]+truth[:, intervals:])*prediction[:, :intervals], axis=1), axis=0)) / (truth.shape[0]-cumulative_censored)
