...
COLOR_ARRAY = ['red', 'indigo', 'cyan', 'pink', 'purple', 'blue', 'chartreuse', 'darkgreen', 'green', 'salmon', 'magenta', 'aquamarine', 'gold', 'aqua',
               'coral', 'tomato', 'grey', 'black', 'maroon', 'hotpink', 'steelblue', 'orange', 'papayawhip', 'lime_green', 'wheat', 'chocolate', 'crimson',
               'slategray', 'violet', 'cadetblue', 'midnightblue', 'darkorchid', 'paleturquoise', 'plum', 'lime', 'teal', 'peru', 'orangered', 'tan',
...
def subplot_scatters(scatters: List[Tuple[np.ndarray, np.ndarray, str, Optional[List[str]]]], prefix: str='./figures/', top_k: int=3, alpha: float=0.5):
    ...
    for prediction, truth, title, paths in scatters:
        ...
        pearson = np.corrcoef(prediction.flatten(), truth.flatten())[1, 0]  # corrcoef returns full covariance matrix
        axes[row, col].text(0, 1, 'Pearson:%0.3f R:%0.3f' % (pearson, (pearson * pearson)), verticalalignment='bottom', transform=axes[row, col].transAxes)
