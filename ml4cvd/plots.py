...
def plot_survival(prediction, truth, title, days_window=1825, prefix='./figures/', paths=None, top_k=3, alpha=0.5):
    ...
    logging.info(f"Prediction  is: {prediction[:5, intervals:intervals+5]} truth  is: {truth[:5, intervals:intervals+5]}")
    predicted_proportion = np.sum(prediction[0 != truth[:, :intervals]], axis=0) / prediction.shape[0]
    ...
    if paths is not None:
        pass
    plt.plot(range(0, days_window, 1+days_window//intervals), 1-predicted_proportion[intervals:], marker='o', label='predicted_proportion')
    plt.plot(range(0, days_window, 1+days_window//intervals), 1-true_proportion[intervals:], marker='o', label='true_proportion')
