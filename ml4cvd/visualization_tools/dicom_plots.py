...
def plot_cardiac_long_axis(b_series, sides=7, fig_width=18, title_prefix=""):
    ...
    fig, axes = plt.subplots(
        sides, sides, figsize=(fig_width, fig_height), facecolor="beige",
    )
    for dcm in b_series:
...
def plot_cardiac_short_axis(
    series, transpose=False, fig_width=18, title_prefix="",
):
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(fig_width, fig_height), facecolor="beige",
    )
    for dcm in series:
...
def plot_mri_series(
    sample_mri, dicoms, series_name, sax_sides, lax_transpose, fig_width,
):
      ...
      <hr><p><pre>{e.message}</pre></p>
        ...
        for dcm_file in os.listdir(tmpdirname):
            ...
            if "cine_segmented_sax_inlinevf" == dcm.SeriesDescription.lower():
                ...
                filtered_dicoms[
                    f"{dcm.SeriesDescription.lower()}_angle_{str(cur_angle)}"
                ].append(dcm)
