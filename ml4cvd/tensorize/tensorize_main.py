...
def parse_args():
    ...
    parser.add_argument(
        "--repo_root", help="Root directory of the cloned ml repo",
    )
    parser.add_argument(
        "--gcp_project",
        default="broad-ml4cvd",
        help="Name of the Google Cloud Platform project",
    )
    parser.add_argument(
        "--gcp_region", default="us-east1", help="Google Cloud Platform region",
    )
...
if __name__ == "__main__":
    ...
    packaging_args = [
        "--requirements_file={}".format(
            f"{args.repo_root}/env/requirements_ml4cvd_dataflow.txt",
        ),
    ...
    tensorize.tensorize_sql_fields(
        pipeline, args.gcs_output_path, args.bigquery_dataset, args.tensor_type,
    )
