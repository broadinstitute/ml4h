...
def _survival_tensor(start_date_key: str,
                     day_window: int,
                     incidence_only: bool = False):
    def _survival_tensor_from_file(tm: TensorMap,
                                   hd5: h5py.File,
                                   dependents=None):
...
cox_mi = TensorMap('myocardial_infarction',
                   Interpretation.TIME_TO_EVENT,
                   tensor_from_file=cox_tensor_from_file('dates/enroll_date'))
cox_mi_incident = TensorMap('myocardial_infarction',
                            Interpretation.TIME_TO_EVENT,
                            tensor_from_file=cox_tensor_from_file(
                                'dates/enroll_date', incidence_only=True))
cox_hyp = TensorMap('hypertension',
                    Interpretation.TIME_TO_EVENT,
                    tensor_from_file=cox_tensor_from_file('dates/enroll_date'))
cox_hyp_incident = TensorMap('hypertension',
                             Interpretation.TIME_TO_EVENT,
                             tensor_from_file=cox_tensor_from_file(
                                 'dates/enroll_date', incidence_only=True))
cox_cad = TensorMap('coronary_artery_disease_soft',
                    Interpretation.TIME_TO_EVENT,
                    tensor_from_file=cox_tensor_from_file('dates/enroll_date'))
cox_cad_incident = TensorMap('coronary_artery_disease_soft',
                             Interpretation.TIME_TO_EVENT,
                             tensor_from_file=cox_tensor_from_file(
                                 'dates/enroll_date', incidence_only=True))
cox_cad = TensorMap('coronary_artery_disease_soft',
