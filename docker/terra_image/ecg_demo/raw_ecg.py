...
def reshape_resting_ecg_to_tidy(sample_id, gcs_folder=None):
  ...
  with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    try:
      tf.io.gfile.copy(
          src=os.path.join(gcs_folder, sample_hd5),
          dst=local_path,
      )
  ...
  with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    with h5py.File(local_path, mode='r') as hd5:
      if 'ecg_bike_recovery' not in hd5:
        print(
            'Warning: Exercise ECG raw signal does not contain ',
      ...
      signal_data['filtered_mV_average'] = (
          1/3.0 * (
              signal_data['filtered_mV_lead_I']
