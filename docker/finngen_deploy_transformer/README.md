...
```
For just getting the embeddings:  
```bash
docker run --rm -v "~/ecg_xml:/work" multimodal_finngen_deploy --mode encode --path /work/xmls --metadata_path /work/ecg_info.tsv --encoder_path /app/encoder_ecg_4096_std.keras --encoder_layer activation_13 --output /work/embeddings_raw.parquet --ecg_input_shape 4096
```
