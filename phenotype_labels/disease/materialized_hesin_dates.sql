WITH oper4 AS (
  SELECT 41200 FieldID, oper.eid, oper4 code,
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.ukbb7089_2023_07_25.hesin_oper` oper
  LEFT JOIN `broad-ml4cvd.ukbb7089_2023_07_25.hesin` h ON oper.eid=h.eid AND oper.ins_index = h.ins_index
  ...
  SELECT 41202 FieldID, hd.eid, diag_icd10 code,
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.ukbb7089_2023_07_25.hesin_diag` hd
  LEFT JOIN `broad-ml4cvd.ukbb7089_2023_07_25.hesin` h ON h.eid = hd.eid and h.ins_index = hd.ins_index
  ...
  SELECT 41210 FieldID, sec_oper.eid, sec_oper.oper4 code, 
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.ukbb7089_2023_07_25.hesin_oper` sec_oper
  LEFT JOIN `broad-ml4cvd.ukbb7089_2023_07_25.hesin` h ON sec_oper.eid=h.eid AND sec_oper.ins_index = h.ins_index
  ...
  SELECT 41204 FieldID, h.eid, sec.diag_icd10 code, 
    CASE
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.ukbb7089_2023_07_25.hesin_diag` sec
  LEFT JOIN `broad-ml4cvd.ukbb7089_2023_07_25.hesin` h ON sec.eid=h.eid AND sec.ins_index = h.ins_index
...
JOIN `broad-ml4cvd.ukbb7089_2023_07_25.phenotype` p ON p.sample_id = diagnostics.eid AND p.array_idx=0 AND p.instance=0 AND p.FieldID=53
