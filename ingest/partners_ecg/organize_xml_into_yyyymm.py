...
def parse_args():
    ...
    parser.add_argument("--path_xml_bad", default="", help="")
...
def organize_xmls():
    ...
    # Debug code
    logging.info("Path to source XMLs is: " + fpath_xml_source)
    logging.info("Path to bad XMLs is: " + fpath_xml_bad)
    ...
    for filename in os.listdir(fpath_xml_source):
        ...
        logging.info("Parsing XML at: {pathToFile}")
        ...
        with open(pathToFile) as fd:
            ...
            try:
                ...
                # If passes date check
                    ...
                    yyyymm = utils.format_date(ecg_date, day_flag=False)
                    fpath_xml_final_yyyymm = os.path.join(fpath_xml_final, yyyymm)

                    # If directory does not exist, create it
                    if os.path.exists(fpath_xml_final_yyyymm):
                        logging.info(f"Valid yyyy-mm directory exists: {fpath_xml_final_yyyymm}")
                    else:
                        logging.info(f"No valid yyyy-mm directory exists. Creating: {fpath_xml_final_yyyymm}")
        ...
        logging.info("XML moved to: " + fpath_xml_final_yyyymm)
